...
*Internal Table for sorting
 ...
 TYPES: BEGIN OF idata ,
          ...
          debit30     LIKE bsik-wrbtr,        "Amount in local curr.
       ...
       LOOP AT lt_data INTO ls_data FROM lv_index.
         IF ls_data-rebzg = itab-belnr AND ls_data-rebzj = itab-gjahr AND ls_data-rebzz = itab-buzei.
           IF ls_data-shkzg = 'H'.
             ...
             itab-debit1 = itab-debit1 - ls_data-wrbtr.     "added by jyoti Currency logic on 23.08.2019
           ELSE.
             itab-debit = itab-debit + ls_data-dmbtr.
             itab-debit1 = itab-debit1 + ls_data-wrbtr.      "added by jyoti Currency logic on 23.08.2019
           ENDIF.
...
*      if itab-shkzg = 'H'.
*        itab-netbal1 = itab-netbal1 * -1.
*       ENDIF.

*       IF itab-waers = 'JPY'.
*        itab-netbal1 =  itab-wrbtr * 100.
*        if itab-shkzg = 'H'.
*        itab-netbal1 = itab-netbal1 * -1.
*       ENDIF.
*        ENDIF.
...
*   PERFORM f_fieldcatalog USING '2'   'LIFNR'     'Vendor code '.
   PERFORM f_fieldcatalog USING '4'   'BELNR'     'Doc No.'.
   PERFORM f_fieldcatalog USING '5'   'BLART'     'Doc Type'.
   PERFORM f_fieldcatalog USING '6'   'BUDAT'     'Posting Date'.
   PERFORM f_fieldcatalog USING '7'   'BLDAT'     'Doc Date'.
   PERFORM f_fieldcatalog USING '8'   'DUEDATE'   'Due Date'.
   PERFORM f_fieldcatalog USING '9'   'GRN_DT'    'GRN Date'.
   PERFORM f_fieldcatalog USING '10'   'VTEXT'    'Payment Terms'.
   PERFORM f_fieldcatalog USING '11'   'CREDIT1'     'Amt Document Currency'.
   PERFORM f_fieldcatalog USING '12'   'WAERS'    'Currency Key'.
   PERFORM f_fieldcatalog USING '13'   'CREDIT'    'Invoice Amt.'.
   PERFORM f_fieldcatalog USING '14'   'DEBIT'    'Payment/Debit Memo Amt'.
   PERFORM f_fieldcatalog USING '15'  'DEBIT1'       'Payment/Debit Memo in Doc. Curr.'.
   PERFORM f_fieldcatalog USING '16'  'NETBAL'    'Net Pending'.
   PERFORM f_fieldcatalog USING '17'  'NETBAL1'    'Net Pending in doc curr'.
   PERFORM f_fieldcatalog USING '18'  'NETB30'    rc13. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '17'  'NETB30_inr'    rc13. "'Net Balance'.
   PERFORM f_fieldcatalog USING '19'  'NETB60'    rc14. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '19'  'NETB60_inr'    rc14. "'Net Balance'.
   PERFORM f_fieldcatalog USING '20'  'NETB90'    rc15. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '20'  'NETB90_inr'    rc15. "'Net Balance'.
   PERFORM f_fieldcatalog USING '21'  'NETB120'   rc16. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '23'  'NETB120_inr'   rc16. "'Net Balance'.
   PERFORM f_fieldcatalog USING '22'  'NETB180'   rc17. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '25'  'NETB180_inr'   rc17. "'Net Balance'.
   PERFORM f_fieldcatalog USING '23'  'NETB360'   rc18. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '27'  'NETB360_inr'   rc18. "'Net Balance'.
   PERFORM f_fieldcatalog USING '24'  'NOT_DUE'   'Not Due'. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '23'  'NETB_N30'   rc19. "'Net Balance'.
   PERFORM f_fieldcatalog USING '25'  'NETB_N60'   rc20. "'Net Balance'.
   PERFORM f_fieldcatalog USING '26'  'NETB_N90'   rc21. "'Net Balance'.
   PERFORM f_fieldcatalog USING '27'  'NETB_N120'  rc22. "'Net Balance'.
   PERFORM f_fieldcatalog USING '28'  'NETB_N180'  rc23. "'Net Balance'.
   PERFORM f_fieldcatalog USING '29'  'NETB_N360'  rc24. "'Net Balance'.
   PERFORM f_fieldcatalog USING '30'  'DAY'       'Over Due Days'.
   PERFORM f_fieldcatalog USING '31'  'PDC'       'PDC Amt.'.
   PERFORM f_fieldcatalog USING '32'  'INVOICE'       'Invoice No.'.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
   ...
   IF sy-subrc = 0.
DATA lv_string_1728 TYPE string.
DATA lv_crlf_1728 TYPE string.
lv_crlf_1728 = cl_abap_char_utilities=>cr_lf.
lv_string_1728 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_1728 lv_crlf_1728 wa_csv INTO lv_string_1728.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_0 TO lv_fullfile.
TRANSFER lv_string_1728 TO lv_fullfile.
