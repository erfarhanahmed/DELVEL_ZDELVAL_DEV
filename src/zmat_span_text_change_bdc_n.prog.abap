...
REPORT ZMAT_SPAN_TEXT_CHANGE_BDC_N.
...
PARAMETERS P_FOLDER LIKE RLGRAP-FILENAME DEFAULT '/Delval/India'."India'."temp/item_master'.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET LV_FULLFILE
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF SY-SUBRC = 0.
DATA lv_string_543 TYPE string.
DATA lv_crlf_543 TYPE string.
lv_crlf_543 = cl_abap_char_utilities=>cr_lf.
lv_string_543 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_543 lv_crlf_543 wa_csv INTO lv_string_543.
  CLEAR: wa_csv.
ENDLOOP.
TRANSFER lv_string_543 TO lv_fullfile.
    CONCATENATE 'File' LV_FULLFILE 'downloaded' INTO LV_MSG SEPARATED BY SPACE.
    MESSAGE LV_MSG TYPE 'S'.
  ENDIF.
