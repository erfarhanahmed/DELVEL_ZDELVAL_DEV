...
types : BEGIN OF ty_keko,
  ...
  TYPES : BEGIN OF ty_keph,
    ...
    material_cost type keph-kst001,
    machine_hour_cost type keph-kst003,
    labour_hour_cost type keph-kst005,
    subcontracting_cost type keph-kst017,
    end of ty_keph.

    data : it_keph type table of ty_keph,
           wa_keph type ty_keph.
*    data : lv_total type chAR255.
TYPES :
  BEGIN OF t_sal_txt,
...
*    sgst          TYPE p DECIMALS 2,        "SGST
    igst          TYPE prcd_elements-kwert,        "JOIG
*    igst          TYPE p DECIMALS 2,        "JOIG
...
*    int_pr5       TYPE prcd_elements-kwert,             "Internal Price
    ...
    tcs_rate      TYPE p DECIMALS 2,              "Tcs RATE
    tcs_amt       TYPE prcd_elements-kwert,              "Tcs Amount
    spart         TYPE vbrk-spart,
    kalnr         type keko-kalnr,
    kadky         type keph-kadky,
    kst001        type keph-kst001,
    ...
    flag     TYPE char1,
    mat_cost TYPE p DECIMALS 2,
    total_c  TYPE keph-kst003,
    vgbel    TYPE vbrp-vgbel,
  END OF t_final,
...
PARAMETERS    : s_werks  TYPE vbrp-werks.
...
*          ls_final-zldfromdate = 'NULL'.
      ...
      IF sy-subrc IS INITIAL.
        ...
        ls_final-spart = ls_sales_inv_hdr-spart.
...
*        ls_final-tcs_p = ls_conditions-kbetr / 10.
      ...
      IF ls_final-spart = '30'.
        ...
        READ TABLE lt_conditions INTO ls_conditions WITH KEY knumv = ls_sales_inv_hdr-knumv
                                                              ...
                                                              kstat = space.
        IF sy-subrc IS  INITIAL.
*          ls_final-tcs_rate = ls_conditions-kbetr / 10.
          ls_final-tcs_rate = ls_conditions-kbetr .
          ls_final-tcs_amt   = ls_final-tcs_amt + ls_conditions-kwert.
        ENDIF.
...
*      IF  ls_final-tcs_amt is INITIAL.
        READ TABLE lt_conditions INTO ls_conditions WITH KEY knumv = ls_sales_inv_hdr-knumv
                                                              kposn = ls_sales_inv_item-posnr
                                                              kschl = 'JWTS'
                                                              kstat = space.
        IF sy-subrc IS  INITIAL.
*          ls_final-tcs_rate = ls_conditions-kbetr / 1000.
          ls_final-tcs_rate = ls_conditions-kbetr .
          ls_final-tcs_amt = ls_final-tcs_amt + ls_conditions-kwert.
        ENDIF.

        IF ls_final-tcs_amt IS INITIAL.
          ls_final-tcs_rate = '0.00'.
        ENDIF.
...
* Implement suitable error handling here
      ...
      wa_final-tcs_amt  = abs( ls_final-tcs_amt ).
      IF ls_final-tcs_amt < 0 .
        CONDENSE wa_final-tcs_amt.
        CONCATENATE '-' wa_final-tcs_amt INTO wa_final-tcs_amt.
      ENDIF.
...
*
*BREAK primus.
      READ TABLE IT_KEKO INTO WA_KEKO WITH KEY MATNR = ls_sales_inv_item-MATNR.

      if sy-subrc = 0.
        ls_final-kalnr = wa_keko-kalnr.
      ENDIF.

sort it_keph by kadky DESCENDING.
READ TABLE IT_KEPH INTO WA_KEPH WITH KEY KALNR = WA_KEKO-kalnr.

IF SY-subrc = 0.
 LS_FINAL-kadky  = WA_KEPH-kadky.
 LS_FINAL-kst001 = WA_KEPH-kst001.
 LS_FINAL-kst003 = WA_KEPH-kst003.
 LS_FINAL-kst005 = WA_KEPH-kst005.
 LS_FINAL-kst017 = WA_KEPH-KST017.

 IF LS_FINAL-kst001 IS INITIAL AND
    LS_FINAL-kst003 IS INITIAL AND
    LS_FINAL-kst005 IS INITIAL AND
    LS_FINAL-kst017 IS INITIAL.

 ls_final-mat_cost = ls_final-stprs1.
 ELSE.
 ls_final-mat_cost = LS_FINAL-kst001 + LS_FINAL-kst003 + LS_FINAL-kst005 + LS_FINAL-kst017.

ENDIF.

* ls_final-material_cost       = wa_keph-kst001.
* ls_final-machine_hour_cost   = wa_keph-kst003.
* ls_final-labour_hour_cost    = WA_KEPH-kst005.
* LS_FINAL-subcontracting_cost = wa_keph-kst017.
* ENDIF.

 ls_final-material_cost       = ls_final-kst001 * ls_sales_inv_item-FKIMG.
 ls_final-machine_hour_cost   = ls_final-kst003 * ls_sales_inv_item-fkimg.
 ls_final-labour_hour_cost    = ls_final-kst005 * ls_sales_inv_item-fkimg.
 ls_final-subcontracting_cost = ls_final-kst017 * ls_sales_inv_item-fkimg.

  ls_final-total_c = ls_final-material_cost + ls_final-machine_hour_cost + ls_final-labour_hour_cost + ls_final-subcontracting_cost.
*ls_final-lv_total = LS_FINAL-material_cost + LS_FINAL-machine_hour_cost + LS_FINAL-labour_hour_cost + LS_FINAL-subcontracting_cost.

ENDIF.

*  IF ls_final-stprs <> ls_final-mat_cost.
*     ls_final-flag = 'X'.
*  ENDIF.

*  ls_final-total_c = ls_final-material_cost + ls_final-machine_hour_cost + ls_final-machine_hour_cost + ls_final-subcontracting_cost.

*      APPEND wa_final TO it_final.

      APPEND ls_final TO ct_final.
"sort ct_final by kadky DESCENDING.
  CLEAR: ls_final, ls_sales_inv_item,ls_sales_inv_hdr,ls_sales_ord_hdr,ls_tvktt,ls_cust_info,ls_mat_master,ls_marc,ls_schedule_line,
         ls_sales_buss,lt_lines,ls_lines,lt1_lines,ls1_lines,blank,des,ls_sal_txt, ls_sal_txt,name,count.

    ENDLOOP.

DELETE ct_final WHERE werks <> s_werks.

*DELETE ct_final WHERE flag = 'X'.
...
*  ls_fieldcat-hotspot   = 'X'.
  ...
  gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'KST001'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Material Cost'."(142).
*  ls_fieldcat-seltext_m = 'Actual Rate'(142).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  ...
  gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'KST003'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Machine Hour Cost'."(142).
*  ls_fieldcat-seltext_m = 'Actual Rate'(142).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  ...
  gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'KST005'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Labour Hour Cost'."(142).
*  ls_fieldcat-seltext_m = 'Actual Rate'(142).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
 ...
 gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'KST017'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Subcontracting Cost'."(142).
*  ls_fieldcat-seltext_m = 'Actual Rate'(142).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
...
gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'MATERIAL_COST'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Material Cost V'."(144).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'MACHINE_HOUR_COST'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Machine Hour Cost V'."(197)."(144).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
  ...
  gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'LABOUR_HOUR_COST'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Labour Hour Cost V'."(198)."(144).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
  ...
  gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'SUBCONTRACTING_COST'.
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Subcontracting cost V'."(199)."(144).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
  ...
  gv_pos = gv_pos + 1.
  ls_fieldcat-fieldname = 'TOTAL_C'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Total C'."(200)."(192).
  ls_fieldcat-col_pos   = gv_pos.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
*  ls_fieldcat-seltext_m = 'Internal Price'(141).
  ls_fieldcat-seltext_m = 'Total Material Cost'."(201)."(192).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'TCS Rate'."(195).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'TCS Amount'."(196).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '10'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Reference No'."(196).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET lv_fullfile
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF sy-subrc = 0.
DATA lv_string_3994 TYPE string.
DATA lv_crlf_3994 TYPE string.
lv_crlf_3994 = cl_abap_char_utilities=>cr_lf.
lv_string_3994 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_3994 lv_crlf_3994 wa_csv INTO lv_string_3994.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_1922 TO lv_fullfile.
TRANSFER lv_string_3994 TO lv_fullfile.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET lv_fullfile
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF sy-subrc = 0.
DATA lv_string_4031 TYPE string.
DATA lv_crlf_4031 TYPE string.
lv_crlf_4031 = cl_abap_char_utilities=>cr_lf.
lv_string_4031 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_4031 lv_crlf_4031 wa_csv INTO lv_string_4031.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_1922 TO lv_fullfile.
