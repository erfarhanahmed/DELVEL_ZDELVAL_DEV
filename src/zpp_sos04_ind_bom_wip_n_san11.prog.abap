...
*&---------------------------------------------------------------------*
REPORT ZPP_SOS04_IND_BOM_WIP_N_SAN11.
...
*         REMARKS  TYPE CHAR18,
         ...
         LGORT    TYPE VBAP-LGORT, "ADDED BY JYOTI ON 11.06.2024
          ZMRP_DATE    TYPe vbap-ZMRP_DATE, "addded by primus jyoti on 12.09.2024
          ZMRP_DATE_NEW    TYPe CHAR12, "addded by primus jyoti on 12.09.2024
...
*         REMARKS  TYPE CHAR18,
         ...
         LGORT    TYPE VBAP-LGORT, "ADDED BY JYOTI ON 11.06.2024
*        ZMRP_DATE   TYPE VBAP-ZMRP_DATE, "addded by primus jyoti on 12.09.2024
        ZMRP_DATE_NEW    TYPE CHAR16, "addded by primus jyoti on 12.09.2024
*         RECEIPT_DATE    TYPE CHAR12,"addded by primus jyoti on 20.12.2023
*         RECEIPTDATE TYPE CHAR12,
       END OF TY_MAIN_DWN.

TYPES : BEGIN OF TY_MAIN_NEW,
...
DATA GT_MAIN_NEW   TYPE TABLE OF TY_MAIN_NEW.
DATA GT_COMPS  TYPE TABLE OF TY_MAIN.
DATA GT_OUTPUT TYPE TABLE OF TY_MAIN.
DATA GS_OUTPUT TYPE  TY_MAIN.
DATA GS_MAIN_NEW TYPE  TY_MAIN_NEW.
DATA GS_MAIN TYPE  TY_MAIN.
...
DATA : LV_VBELN(10) TYPE C."""ADDED BY SNEHAL 23/08/2024
DATA : LV_posnr(06) TYPE C."""ADDED BY SNEHAL 23/08/2024
DATA :   v_str                 TYPE string,
           vv_file               TYPE ibipparms-path.
*DATA : LV_VBELN TYPE VBELN.
DATA : GT_MAIN_NEW1 TYPE  table of ty_main_new.
data : gs_main_new1 TYPE ty_main_new.
...
PARAMETERS : S_FILE TYPE RLGRAP-FILENAME." DEFAULT 'e:/Delval/India'."India'."temp/item_master'.
*'C:\Users\user\Downloads\zsos.xlsx'.      "ADDED BY SNEHAL 23/08/2024
...
*PARAMETERS P_FOLDER LIKE RLGRAP-FILENAME DEFAULT 'e:\delval\temp'.  "H:\planning\actuator'  ##NO_TEXT
*.
*SELECTION-SCREEN END OF BLOCK B5.


SELECTION-SCREEN BEGIN OF BLOCK B3 WITH FRAME TITLE TEXT-074 .
PARAMETERS P_DOWN AS CHECKBOX.
*PARAMETERS P_FOLDER LIKE RLGRAP-FILENAME DEFAULT 'e:\delval\temp'.  "H:\planning\actuator'  ##NO_TEXT
PARAMETERS P_FOLDER LIKE RLGRAP-FILENAME DEFAULT '/Delval/India'."temp'.  "H:\planning\actuator'  ##NO_TEXT
.
SELECTION-SCREEN END OF BLOCK B3.
...
SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT 5(63) TEXT-007.
SELECTION-SCREEN END OF LINE.


"""""""""""""""added by snehal27/08/2024""""""""""""
...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR S_FILE.    "ADDED BY SNEHAL 23/08/2024
...
*
*  CALL FUNCTION 'TEXT_CONVERT_XLS_TO_SAP'   " ADDED BY SNEHAL 23/08/2024
*    EXPORTING
**     I_FIELD_SEPERATOR    =
*      I_LINE_HEADER        = 'X'
...
*      I_FILENAME           = S_FILE
*    TABLES
*      I_TAB_CONVERTED_DATA = GT_MAIN_NEW
*    EXCEPTIONS
*      CONVERSION_FAILED    = 1
*      OTHERS               = 2.
*  IF SY-SUBRC <> 0.
     ...
     DO.
      ...
      IF sy-subrc = 0.
        SPLIT l_wa_raw_data AT cl_abap_char_utilities=>horizontal_tab INTO
         gs_main_new-vbeln
         gs_main_new-posnr.
         APPEND gs_main_new TO gt_main_new.
      ELSE.
        EXIT.
     ENDIF.
...
*    CLEAR: wa_err0r.
  ENDIF.
  """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
  IF GT_MAIN_NEW IS  NOT INITIAL.
*

      LOOP AT GT_MAIN_NEW INTO GS_MAIN_NEW .    """"added by snehal 27/08/2024

     if sy-tabix NE '1'.
      LV_VBELN = GS_MAIN_NEW-VBELN.
      ...
      GS_MAIN_NEW1-VBELN = LV_VBELN.

        LV_posnr = GS_MAIN_NEW-posnr.
      ...
      GS_MAIN_NEW1-posnr = LV_posnr.
      APPEND  GS_MAIN_NEW1 to  Gt_MAIN_NEW1.
      endif.
       ENDLOOP.
endif.


if GT_MAIN_NEW1 IS  NOT INITIAL.
      SELECT A~VBELN A~POSNR A~MATNR
              ...
              A~ZMRP_dATE "ZMRP_DATE ADDED BY JYOTI ON 12.09.2024
              D~EDATU D~WMENG D~ETENR A~LFSTA
              F~OMENG
              G~PLIFZ
...
*                        AND C~POSNR = A~POSNR
          ...
          FOR ALL ENTRIES IN GT_MAIN_NEW1
          WHERE A~VBELN = GT_MAIN_NEW1-vbeln                ""GT_MAIN_NEW-VBELN
             AND A~POSNR = GT_MAIN_NEW1-POSNR        "added by snehal ankush date 19/08/2024
            AND A~WERKS = P_WERKS
            AND A~LFSTA NE 'C'
...
*        AND A~POSNR IN S_POSNR        "added by snehal ankush date 19/08/2024
*       AND A~WERKS = P_WERKS
*       AND C~LFSTA NE 'C'
*       AND A~MATNR IN S_MATNR
*       AND D~EDATU IN S_EDATU
...
*SELECT vbeln
*  FROM vbap
*  INTO TABLE LT_VBAP FOR ALL ENTRIES IN LT_vbak
*  WHERE vbeln = LT_vbak-vbeln.
*
*if sy-subrc = 0.
*   READ TABLE lt_vbap INTO ls_vbap WITH KEY posnr = LS_vbap-posnr VBELN = LS_VBAK-VBELN.
*   endif.
...
*>
  ...
  LOOP AT GT_MAIN INTO LS_COMPS.
    ...
    LS_COMPS-ZMRP_DATE = LS_COMPS-ZMRP_DATE."added by jyoti on 12.09.2024
...
*  SORT GT_COMPS BY EDATU VBELN MATNR LEVEL POSNR. "Added By Snehal Rajale On 28.01.2021"COMMENTED BY JYOTI ON 12.09.2024
  SORT GT_COMPS BY ZMRP_DATE VBELN MATNR LEVEL POSNR. "Added By JYOTI On 28.01.2021
*  SORT GT_COMPS BY ZMRP_DATE VBELN MATNR LEVEL POSNR. "Added By JYOTI On 28.01.2021
*  sort gt_comps by ZMRP_DATE ASCENDING.
...
*BREAK-POINT.
types: BEGIN OF ty_vend_qty,
       matnr TYPE matnr,
       SLLAB TYPE LABST,
       END OF ty_vend_qty.
DATA it_vend_tot_uqty TYPE TABLE of ty_vend_qty.
 SELECT  * "matnr,
            "SLLAB
    FROM MSSL
    INTO TABLE @DATA(it_vend_qty)
    FOR ALL ENTRIES IN @GT_LIST
    WHERE matnr = @GT_LIST-IDNRK.
sort it_vend_qty by matnr.
...
* Implement suitable error handling here
  ...
  SELECT SINGLE
         SUM( SLLAB ) AS vendor_qty
         FROM @it_vend_qty AS A
         WHERE matnr = @<FS_LIST>-IDNRK
         INTO  @DATA(wa_vendor_quan) .
IF sy-subrc = 0.
 <FS_LIST>-UNSTK = <FS_LIST>-UNSTK - wa_vendor_quan.
 CLEAR:wa_vendor_quan.
ENDIF.
...
*  WRITE: / 'SO Date         From', S_EDATU-LOW, 'To', S_EDATU-HIGH.
  ...
  IF SY-SUBRC = 0.
DATA lv_string_1546 TYPE string.
DATA lv_crlf_1546 TYPE string.
lv_crlf_1546 = cl_abap_char_utilities=>cr_lf.
lv_string_1546 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_1546 lv_crlf_1546 wa_csv INTO lv_string_1546.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_1008 TO lv_fullfile.
TRANSFER lv_string_1546 TO lv_fullfile.
...
*  lv_file = 'SOSTEST.TXT'.
*  lv_file = 'SOS_DATE_CHNG.TXT'.
  LV_FILE = 'ZSOS_MILIND.TXT'.   "Added By Nilay B. On 17.06.2023
...
*  WRITE: / 'SO Date         From', S_EDATU-LOW, 'To', S_EDATU-HIGH.
  ...
  """"
* clear  LV_FULLFILE.
*   CONCATENATE '/Delval/India' '' into LV_FULLFILE.
*  """"

  OPEN DATASET LV_FULLFILE
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF SY-SUBRC = 0.
DATA lv_string_1602 TYPE string.
DATA lv_crlf_1602 TYPE string.
lv_crlf_1602 = cl_abap_char_utilities=>cr_lf.
lv_string_1602 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_1602 lv_crlf_1602 wa_csv INTO lv_string_1602.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_1008 TO lv_fullfile.
TRANSFER lv_string_1602 TO lv_fullfile.
*    CLOSE DATASET LV_FULLFILE.
    CONCATENATE 'File' LV_FULLFILE 'downloaded'   ##NO_TEXT
      INTO LV_MSG SEPARATED BY SPACE.
    MESSAGE LV_MSG TYPE 'S'.
...
* 'Remarks'
 ...
 'Targated MRP Date'   "added by jyoti on 12.09.2024
...
*   Final
    ...
    DATA : ZMRP_dATE TYPE CHAR16.
     IF  <FS_COMPS>-ZMRP_DATE IS NOT INITIAL.
        CALL FUNCTION 'CONVERSION_EXIT_IDATE_OUTPUT'
          EXPORTING
            INPUT  = <FS_COMPS>-ZMRP_DATE
          IMPORTING
            OUTPUT = <FS_COMPS>-ZMRP_dATE_NEW .
        CONCATENATE  <FS_COMPS>-ZMRP_dATE_NEW+0(2)  <FS_COMPS>-ZMRP_dATE_NEW+2(3) <FS_COMPS>-ZMRP_dATE_NEW+5(4)
                    INTO <FS_COMPS>-ZMRP_dATE_NEW SEPARATED BY '-'.
      ELSE.
        <FS_COMPS>-ZMRP_dATE_NEW  = ''.
      ENDIF.
...
FORM GT_OUTPUT1 .
  ...
  SORT GT_OUTPUT BY zmrp_date vbeln matnr level posnr.
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  VEND
*&---------------------------------------------------------------------*
...
*&---------------------------------------------------------------------*
  ...
  ls_data-field1 =  'Sales Order'.
  ls_data-field2 =  'Sales Order Line Item'.
...
* CREATE FIRST EXCEL SHEET
  ...
  sheet
  'ACTIVATE'.
  SET PROPERTY OF sheet 'NAME' = 'sales doc no and line item'.
*&---------------------------------------------------------------------*
