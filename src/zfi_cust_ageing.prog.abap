...
TYPES: BEGIN OF TY_T052U ,     " added by ps
         ZTERM TYPE T052U-ZTERM,
         TEXT1 TYPE T052U-TEXT1,
       END OF TY_T052U.

DATA: IT_T052U TYPE STANDARD TABLE OF TY_T052U.  "added by ps
DATA: WA_T052U TYPE TY_T052U.
...
*         zbd1t     LIKE bsid-zbd1t,         "Cash Discount Days 1
         ...
         LAND1      TYPE KNA1-REGIO,
         REG_DESC   TYPE T005U-BEZEI,
         PAY_TERM   TYPE CHAR80,  " add by ps
       END OF IDATA.
...
*    SELECT SINGLE name1 INTO itab-name1 FROM kna1 WHERE kunnr = itab-kunnr.                  "commented by sonu
    SELECT SINGLE NAME1 REGIO LAND1 INTO ( ITAB-NAME1 , ITAB-REGIO , ITAB-LAND1 ) FROM KNA1 WHERE KUNNR = ITAB-KUNNR.
    SELECT SINGLE BEZEI INTO ITAB-REG_DESC FROM T005U WHERE BLAND = ITAB-REGIO AND LAND1 = ITAB-LAND1 AND SPRAS = 'EN'.    " added by sonu
...
***********Net Balance
    ...
    READ TABLE IT_T052U INTO WA_T052U WITH KEY ZTERM = ITAB-ZTERM.   "add by ps
    IF SY-SUBRC = 0.
      ITAB-PAY_TERM = WA_T052U-TEXT1.
    ENDIF.

    IF LS_TVZBT-VTEXT IS INITIAL and ITAB-PAY_TERM IS NOT INITIAL.
   CONCATENATE LS_TVZBT-ZTERM ITAB-PAY_TERM INTO ITAB-VTEXT SEPARATED BY SPACE.
    ENDIF.
    IF LS_TVZBT-VTEXT IS NOT INITIAL and ITAB-PAY_TERM IS INITIAL..
       ITAB-PAY_TERM = LS_TVZBT-VTEXT.
    ENDIF.
  ...
  IF P_DOWN = 'X'.
    ...
    LOOP AT ITAB.
      ...
      WA_DOWN-REGIO    = ITAB-REGIO   .
      WA_DOWN-REG_DESC    = ITAB-REG_DESC   .        " added by sonu
      WA_DOWN-PAY_TERM  = ITAB-PAY_TERM.  " added by ps
...
FORM FILL_FIELDCATALOG.
  ...
  PERFORM F_FIELDCATALOG USING '33'  'REGIO'      'Region'.                           " added by sonu
  PERFORM F_FIELDCATALOG USING '34'  'REG_DESC'   'Region Description'.               " added by sonu
  PERFORM F_FIELDCATALOG USING '35'  'PAY_TERM'   'Payment Term Long text'.            " added by ps
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  LV_FILE = 'ZCUST_AGE.TXT'.


  CONCATENATE P_FOLDER '/' SY-DATUM SY-UZEIT LV_FILE
    INTO LV_FULLFILE.

  WRITE: / 'ZCUST_AGE REPORT started on', SY-DATUM, 'at', SY-UZEIT.
  OPEN DATASET LV_FULLFILE
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF SY-SUBRC = 0.
    DATA LV_STRING_1485 TYPE STRING.       "commented by sonu
    DATA LV_CRLF_1485 TYPE STRING.         "commented by sonu
    LV_CRLF_1485 = CL_ABAP_CHAR_UTILITIES=>CR_LF.    "commented by sonu
    LV_STRING_1485 = HD_CSV.           "commented by sonu
*TRANSFER hd_csv TO lv_fullfile.   "added by sonu
    LOOP AT IT_CSV INTO WA_CSV.       "added by sonu
*  IF sy-subrc = 0.                 "added by sonu
*        TRANSFER wa_csv TO lv_fullfile.                        "added by sonu
*         ENDIF.                                                 "added by sonu
      CONCATENATE LV_STRING_1485 LV_CRLF_1485 WA_CSV INTO LV_STRING_1485.    " commented by sonu
      CLEAR: WA_CSV.                                                         " commented by sonu
    ENDLOOP.
*TRANSFER lv_string_1762 TO lv_fullfile.
    TRANSFER LV_STRING_1485 TO LV_FULLFILE.                         "commented by sonu
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET LV_FULLFILE
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF SY-SUBRC = 0.
    DATA LV_STRING_1523 TYPE STRING.                   " commented by sonu
    DATA LV_CRLF_1523 TYPE STRING.                     " commented by sonu
    LV_CRLF_1523 = CL_ABAP_CHAR_UTILITIES=>CR_LF.      " commented by sonu
    LV_STRING_1523 = HD_CSV.                           " commented by sonu
*    TRANSFER hd_csv TO lv_fullfile.                      "added by sonu
    LOOP AT IT_CSV INTO WA_CSV.
      CONCATENATE LV_STRING_1523 LV_CRLF_1523 WA_CSV INTO LV_STRING_1523.  " commented by sonu
      CLEAR: WA_CSV.                                                        " commented by sonu
*  IF sy-subrc = 0.                                              "added by sonu
*        TRANSFER wa_csv TO lv_fullfile.                         "added by sonu
*        endif.                                           "added by sonu
    ENDLOOP.
*TRANSFER lv_string_1762 TO lv_fullfile.
    TRANSFER LV_STRING_1523 TO LV_FULLFILE.          " commented by sonu
    CONCATENATE 'File' LV_FULLFILE 'downloaded' INTO LV_MSG SEPARATED BY SPACE.
    MESSAGE LV_MSG TYPE 'S'.
  ENDIF.
...
FORM CVS_HEADER  USING    PD_CSV.
  ...
  CONCATENATE 'GL Type'
              ...
              'Over Due Days'
              'Refresh Date'
              'Region'                          " added by sonu
              'Region Description'              " added by sonu
              'Payment Term Long Text'          " added by ps
*              ''
              INTO PD_CSV
...
*        CONCATENATE
        ...
        W_LINE-REGIO           =  WA_DOWN-REGIO.
        W_LINE-REG_DESC        =  WA_DOWN-REG_DESC.
        W_LINE-PAY_TERM        =  WA_DOWN-PAY_TERM.
      ...
      LS_HEADINGS-LINE  = 'Region'.                  " added by sonu
      APPEND LS_HEADINGS TO LT_HEADINGS.
      CLEAR LS_HEADINGS.

      LS_HEADINGS-LINE  = 'Region Description'.      " added by sonu
      APPEND LS_HEADINGS TO LT_HEADINGS.
      CLEAR LS_HEADINGS.

      LS_HEADINGS-LINE  = 'Payment Term Long text'.      " add by PS
      APPEND LS_HEADINGS TO LT_HEADINGS.
      CLEAR LS_HEADINGS.
