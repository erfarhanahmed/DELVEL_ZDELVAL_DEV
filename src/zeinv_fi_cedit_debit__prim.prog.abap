*&---------------------------------------------------------------------*
*& Report ZEINV_FI_CEDIT_DEBIT
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zeinv_fi_cedit_debit__prim MESSAGE-ID zinv_message.
 ...
 ENDIF.
  ...
  IF  lt_bkpf IS NOT INITIAL.
    ...
    SELECT
      ...
      lt_bkpf WHERE docno = lt_bkpf-belnr AND doc_year = lt_bkpf-gjahr.
  ...
  CASE sy-ucomm.
    WHEN '&IRN'.
      ...
      SELECT belnr zzstatus FROM zeinv_res_fb70 INTO TABLE lt_zeinv_c
        FOR ALL ENTRIES IN lt_final
        WHERE belnr = lt_final-belnr AND
              gjahr = lt_final-gjahr.          "Added By SR On 20.04.2021
...
*          ls_final-zzstatus = 'N'.
          ...
          CALL FUNCTION 'ZJSON_FILE_FB70'                  "'ZJSON_FILE_CONVERSION'
            EXPORTING
...
*lv_url2 = 'http://35.154.208.8:8080/einvoice/v1.03/authentication'."DEV    "'http://35.154.208.8:8080/einvoice/v1.00/authentication'.
...
*wa_auth-username = 'perennialsystems'. " DEV
*wa_auth-password = 'p3r3nn!@1'. " DEV
