...
DATA : IT_MSEG_104 TYPE STANDARD TABLE OF TY_MSEG,
       WA_MSEG_104 TYPE TY_MSEG.

DATA : IT_MSEG_106 TYPE STANDARD TABLE OF TY_MSEG,
...
DATA: IT_DATA_NEW TYPE STANDARD TABLE OF TY_DATA,
      WA_DATA_NEW TYPE TY_DATA.
...
*          TRAN_QTY_V    TYPE MSEG-DMBTR,
          TRAN_QTY_V_new    TYPE MSEG-DMBTR,"ADDED BY JYOTI ON 01.07.2024
...
*          LABST_V     TYPE mseg-dmbtr,
          ...
          ERSDA         TYPE MARA-ERSDA,
          MTART         TYPE MARA-MTART,
         MENGE_104    TYPE MSEG-MENGE,"ADDED BY JYOTI ON 28.06.2024
         QTY_104_VAL  type mseg-dmbtr,"Added by jyoti on 28.06.2024
*         TRAN_QTY_new   TYPE P DECIMALS 0,"ADDED BY JYOTI ON 28.06.2024
*         TRAN_QTY_V_new TYPE MSEG-DMBTR,"ADDED BY JYOTI ON 28.06.2024
          MENGE_106    TYPE MSEG-MENGE,
        END OF TY_FINAL.
...
PARAMETERS P_FOLDER LIKE RLGRAP-FILENAME DEFAULT '/Delval/USA'."USA'."USA'."USA'."usa'.  "'/Delval/USA'."usa'.
...
*********************************************************
   SELECT * from VBUP
      INTO table @DATA(it_vbup_1)
      where LFSTA  NE 'C'.



if it_vbup_1 is NOT INITIAL.
       select vbeln
         posnr
         matnr
         werks
    ...
    FROM  VBAP
     FOR ALL ENTRIES IN it_vbup_1
      WHERE vbeln = it_vbup_1-vbeln
      and posnr = it_vbup_1-posnr
      AND   WERKS  IN PLANT
      AND   ABGRU = ''.
 endif.
* ******************************************************
...
*      AND   B~LFSTA  NE 'C'.
  ...
  IF  IT_DATA_NEW IS NOT INITIAL.                              "  ADDED BY PRATYANCHA ON 27.06.2024
    SELECT
    VBELN
    VBTYP
    KNUMV
    FROM VBAK
    INTO TABLE IT_VBAK
    FOR ALL ENTRIES IN IT_DATA_NEW
    WHERE VBELN = IT_DATA_NEW-VBELN
    AND   VBTYP IN ( 'C' , 'I' , 'H' ).
  ENDIF.                                                    "  ADDED BY PRATYANCHA ON 27.06.2024
  ...
  IF IT_DATA_NEW IS NOT INITIAL.                                                           "  ADDED BY PRATYANCHA ON 27.06.2024
    SELECT
      ...
      INTO TABLE IT_LIPS
             FOR ALL ENTRIES IN IT_DATA_NEW
             WHERE VGBEL = IT_DATA_NEW-VBELN
              AND  VGPOS = IT_DATA_NEW-POSNR.
  ENDIF.                                                        "  ADDED BY PRATYANCHA ON 27.06.2024
...
*
    ...
    LOOP AT IT_DATA_NEW INTO WA_DATA_NEW WHERE MATNR = WA_MARA_MARD-MATNR AND WERKS = WA_MARA_MARD-WERKS.
      CLEAR: QTY,PRICE,PENDING,DEL.
      WA_FINAL-VBELN = WA_DATA_NEW-VBELN.
      WA_FINAL-POSNR = WA_DATA_NEW-POSNR.
      READ TABLE IT_VBAK INTO WA_VBAK WITH KEY VBELN = WA_DATA_NEW-VBELN.
      IF SY-SUBRC = 0.
        WA_FINAL-VBAP_QTY = WA_FINAL-VBAP_QTY + WA_DATA_NEW-KWMENG.
        QTY = QTY + WA_DATA_NEW-KWMENG.
      ENDIF.

      READ TABLE IT_KONV INTO WA_KONV WITH KEY KNUMV = WA_VBAK-KNUMV KPOSN = WA_DATA_NEW-POSNR.
      IF SY-SUBRC = 0.
...
*********************** GRN Amount ********************************
    ...
    DATA:TRAN_VAL  TYPE MSEG-DMBTR,
          MSEG_QTY_104_VAL type mseg-dmbtr,
         TRAN_QTY  TYPE MSEG-MENGE,
         TRAN_QTY_new  TYPE MSEG-MENGE,
         MSEG_QTY  TYPE MSEG-MENGE,
         TRAN_VAL_new  TYPE MSEG-DMBTR,
          MSEG_QTY_104  TYPE MSEG-MENGE,"Added by jyoti on 28.06.2024
          MSEG_QTY_106  TYPE MSEG-MENGE,"Added by jyoti on 28.06.2024
         MSEG_QTY1 TYPE MSEG-MENGE.


    LOOP AT IT_TRANSIT INTO WA_TRANSIT WHERE MATNR = WA_MARA_MARD-MATNR AND WERKS = WA_MARA_MARD-WERKS..
      CLEAR: TRAN_VAL,TRAN_QTY,MSEG_QTY ,MSEG_QTY1,MSEG_QTY_104, MSEG_QTY_104_VAL,TRAN_QTY_new,TRAN_VAL_new,MSEG_QTY_106.
      ...
      ENDLOOP.
********************************************aDDED BY JYOTI ON 28.06.2024*********************************
      LOOP AT IT_MSEG_104 INTO WA_MSEG_104 WHERE EBELN = WA_TRANSIT-EBELN AND EBELP = WA_TRANSIT-EBELP AND
                                       WERKS = WA_TRANSIT-WERKS and MATNR = WA_TRANSIT-MATNR.
        IF WA_MSEG_104-SMBLN IS INITIAL.
          WA_FINAL-MENGE_104 =  WA_FINAL-MENGE_104  + WA_MSEG_104-MENGE.
           MSEG_QTY_104   = MSEG_QTY_104  + WA_MSEG_104-MENGE.
        ENDIF.
      ENDLOOP.
***********************************************************************************************************
      LOOP AT IT_MSEG_105 INTO WA_MSEG_105 WHERE  EBELN = WA_TRANSIT-EBELN AND EBELP = WA_TRANSIT-EBELP AND
        ...
        ENDIF.
      ENDLOOP.
******************************************added by jyoti on 29.06.2024***********************************
*       LOOP AT IT_MSEG_106 INTO WA_MSEG_104 WHERE EBELN = WA_TRANSIT-EBELN AND EBELP = WA_TRANSIT-EBELP AND
*                                       WERKS = WA_TRANSIT-WERKS and MATNR = WA_TRANSIT-MATNR.
*          WA_FINAL-MENGE_106 =  WA_FINAL-MENGE_106  + WA_MSEG_106-MENGE.
*           MSEG_QTY_106   = MSEG_QTY_106  + WA_MSEG_106-MENGE.
*      ENDLOOP.
****************************************************************************************************
      ...
      TRAN_QTY_new = MSEG_QTY - MSEG_QTY1 - MSEG_QTY_104 ."+ MSEG_QTY_106 .

      IF MSEG_QTY IS NOT INITIAL.
        TRAN_VAL = TRAN_QTY * WA_TRANSIT-NETPR.
        TRAN_VAL_new = TRAN_QTY_new * WA_TRANSIT-NETPR.
      ENDIF.
*      WA_FINAL-TRAN_QTY_V = WA_FINAL-TRAN_QTY_V + TRAN_VAL.
      WA_FINAL-TRAN_QTY_V_new = WA_FINAL-TRAN_QTY_V_new + TRAN_VAL_new.

*********Added by jyoti on28.06.2024 for 104 qty value***************
        IF MSEG_QTY_104 IS NOT INITIAL.
*          wa_final-QTY_104_VAL  = MSEG_QTY_104 *  WA_FINAL-VALUE.
          MSEG_QTY_104_VAL = MSEG_QTY_104 * WA_FINAL-VALUE.
*          wa_final-QTY_104_VAL = wa_final-QTY_104_VAL + MSEG_QTY_104_VAL.
        endif.
         wa_final-QTY_104_VAL = wa_final-QTY_104_VAL + MSEG_QTY_104_VAL.
************************************************************************

    ENDLOOP.
    CLEAR WA_FINAL-TRAN_QTY_NEW.
*    WA_FINAL-TRAN_QTY = WA_FINAL-MSEG_MENGE - WA_FINAL-MSEG_MENGE1.
    WA_FINAL-TRAN_QTY_new = WA_FINAL-MSEG_MENGE - WA_FINAL-MSEG_MENGE1 - WA_FINAL-MENGE_104 ."+ WA_FINAL-MENGE_106.
    ...
    WA_FINAL-SO_FALL_QTY =  WA_FINAL-OPEN_QTY - WA_FINAL-LABST - WA_FINAL-TRAN_QTY_NEW .
...
*    COLLECT WA_FINAL INTO IT_FINAL.
    ...
    CLEAR : WA_FINAL,LS_FINAL,WA_DATA_NEW.
  ENDLOOP.
...
*      LS_SORT-TRAN_QTY_V     = LS_SORT-TRAN_QTY_V  + WA_FINAL-TRAN_QTY_V.
         LS_SORT-TRAN_QTY_new       = LS_SORT-TRAN_QTY_new    + WA_FINAL-TRAN_QTY_new.
      LS_SORT-TRAN_QTY_V_new     = LS_SORT-TRAN_QTY_V_new  + WA_FINAL-TRAN_QTY_V_new.
      ...
      LS_SORT-MENGE_104          = LS_SORT-MENGE_104       + WA_FINAL-MENGE_104."added by jyoti on 29.06.2024
      LS_SORT-QTY_104_VAL          = LS_SORT-QTY_104_VAL       + WA_FINAL-QTY_104_VAL."added by jyoti on 29.06.2024
