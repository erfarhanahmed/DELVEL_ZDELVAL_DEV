...
*&
*&---------------------------------------------------------------------*
REPORT zsu_mm_upload_mat_mast_new NO STANDARD PAGE HEADING LINE-SIZE 255.
...
*          j_1iassval  TYPE j_1iassval-j_1ivalass,  "82 Assessable Value
          text_line   TYPE CHAR200,      "83 Long text
          qmpur       TYPE qmpur,        "84 QM in Procurement is Active
          ssqss       TYPE ssqss,        "85 QA Control Key
          art         TYPE rmqam-art,    "86 Inspection Type
          aktiv       TYPE rmqam-aktiv,  "87 QA Inspection Active
          pstat       TYPE pstat_d,      "88 Maintenance status
          document    TYPE bapi_mara-document,    "89Document
          doc_vers    TYPE bapi_mara-doc_vers,    "90Document version
*          NO_COSTING  TYPE bapi_marc-NO_COSTING,"91
          MSS         TYPE mara-ZZMSS,  "92
          EDS         TYPE mara-ZZEDS,  "93
           mixed_mrp   TYPE bapi_marc-mixed_mrp,  "94 Mixed MRP indicator
...
*      WHERE matnr = wa_file-matnr.
      ...
      SELECT SINGLE matnr FROM marc INTO lv_matnr
      WHERE matnr = wa_file-matnr AND WERKS = wa_file-werks.
    IF p_updt = 'X'.
      sy-subrc = 1.
    ENDIF.
...
*wa_valuationdata-PLNDPRICE3 =  WA_FILE-PRICE3.
*wa_valuationdatax-PLNDPRICE3 = 'X'.
*BREAK primusabap.
wa_valuationdata-PLNDPRDATE3 = SY-DATUM.     "WA_FILE-PLNDPRDATE3.
...
IF wa_plantdata-proc_type is NOT INITIAL .
...
*    SUBMIT  zmm_matcin_upload AND RETURN.
    ...
    ENDIF.
********    CODE ADDED BY SAGAR DEV 23-01-2024
    IF t_longtext IS NOT INITIAL .

    SUBMIT zus_mm_upload_mat_long_txt AND RETURN.

    ENDIF.
*    IF t_longtext  IS  NOT INITIAL .  " CODE COMMENTED BY 23-01-2024
...
*    CLEAR: wa_err0r.
 ...
 ELSE.
   MOVE wa_file-matnr TO i_error-matnr.
    MOVE 'Procurement type is missing.' TO i_error-l_mstring.
    APPEND i_error .
...
**  w_tax-taxclass_4 = wa_file-taxm4.
  ...
  w_tax-depcountry = w_land1.
  w_tax-tax_type_1 = 'MWAS'.    "'JTX1' ."w_mwst.
