...
*&
*&---------------------------------------------------------------------*
REPORT ZQM_STOCK_REVERSAL_DATE.
*{   INSERT         DEVK923946                                        1
TYPES:
  T_MKPF_TAB  LIKE MKPF  OCCURS 0,
  T_MSEG_TAB  LIKE MSEG  OCCURS 0.
SELECTION-SCREEN:  BEGIN OF BLOCK p WITH FRAME TITLE text-009.
PARAMETERS:
  PRUEFLOS LIKE QALS-PRUEFLOS OBLIGATORY MEMORY ID QLS,
  S_BUDAT LIKE QALS-BUDAT OBLIGATORY.
...
*  ************************** ADDED CODE BY VIVEK
  ...
  ELSE.
  L_IMKPF-BUDAT = SY-DATLO = S_BUDAT.
  ENDIF.
*  *************************** END CODE.
