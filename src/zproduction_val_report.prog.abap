...
SELECTION-SCREEN BEGIN OF BLOCK B2 WITH FRAME TITLE ABC .
  ...
  PARAMETERS P_FOLDER TYPE RLGRAP-FILENAME DEFAULT '/Delval/India'."India'."temp'.         "'/Delval/India'."temp'
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET LV_FULLFILE
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF SY-SUBRC = 0.
    DATA LV_STRING_1650 TYPE STRING.
    DATA LV_CRLF_1650 TYPE STRING.
    LV_CRLF_1650 = CL_ABAP_CHAR_UTILITIES=>CR_LF.
    LV_STRING_1650 = HD_CSV.
    LOOP AT IT_CSV INTO WA_CSV.
      CONCATENATE LV_STRING_1650 LV_CRLF_1650 WA_CSV INTO LV_STRING_1650.
      CLEAR: WA_CSV.
    ENDLOOP.
*TRANSFER lv_string_778 TO lv_fullfile.
    TRANSFER LV_STRING_1650 TO LV_FULLFILE.
    CONCATENATE 'File' LV_FULLFILE 'downloaded' INTO LV_MSG SEPARATED BY SPACE.
    MESSAGE LV_MSG TYPE 'S'.
  ENDIF.
