...
CONCATENATE wa_hdr-aedat+0(4) wa_hdr-aedat+4(2) wa_hdr-aedat+6(2)
INTO wa_hdr-aedat SEPARATED BY '-'.

CONCATENATE wa_hdr-bedat+0(4) wa_hdr-bedat+4(2) wa_hdr-bedat+6(2)
INTO wa_hdr-bedat SEPARATED BY '-'.
*BREAK-POINT.
...
SELECT SINGLE MAX( udate )
  ...
  GV_UDATE =  wa_hdr-udate.


  CONCATENATE GV_UDATE+0(4) GV_UDATE+4(2) GV_UDATE+6(2)
INTO GV_UDATE SEPARATED BY '-'.
*BREAK-POINT.
...
SELECT SINGLE J_1BBRANCH
  ...
  SELECT SINGLE GSTIN
    ...
    IF gv_gst is INITIAL.
  gv_gst = '27AACCD2898L1Z4'.
ENDIF.

*BREAK-POINT.
SELECT SINGLE addrnumber  name1 city1
   post_code1  street  country
...
IF sy-subrc = 0.
  ...
  SELECT SINGLE smtp_addr FROM adr6
    ...
    WHERE addrnumber = wa_hdr-p_adrnr
    and   CONSNUMBER = '2'.    "wa_hdr-CONSNUMBER.  "lv_adrnr.
...
SELECT SINGLE adrnr name1 regio STCD3
  FROM lfa1
  INTO (wa_hdr-v_adrnr, wa_hdr-v_name1, GS_REGION , wa_hdr-stcd3)
  WHERE lifnr = wa_hdr-lifnr.

  SELECT SINGLE stcd3 regio FROM kna1
  into (WA_STCD3, gst_region) WHERE lifnr = wa_lifnr.


  SELECT SINGLE * FROM lfm1 INTO wa_lfm1
    WHERE lifnr = wa_hdr-lifnr.
IF  gst_region is not INITIAL.
...
CLEAR : LV_UDATE,LV_zeivr.
refresh :IT_CDPOS,IT_CDHDR.
...
*    wa_item-zeivr =  wa_mara-zeivr.
    gv_custom_mat = wa_mara-wrkst.
  ENDIF.

select SINGLE UDATE from cdhdr INTO LV_UDATE WHERE OBJECTID = WA_HDR-EBELN.
  SELECT OBJECTID CHANGENR FROM CDPOS INTO TABLE IT_CDPOS WHERE OBJECTID = wa_item-MATNR
AND TABNAME = 'MARA' AND FNAME = 'ZEIVR'.
    IF IT_CDPOS IS NOT INITIAL.
    SELECT OBJECTID CHANGENR FROM cdhdr INTO TABLE IT_CDHDR FOR ALL ENTRIES IN IT_CDPOS  WHERE OBJECTID = wa_item-MATNR
      ...
      ENDIF.
      SELECT SINGLE VALUE_OLD FROM CDPOS INTO LV_zeivr WHERE OBJECTID = wa_item-MATNR
        AND TABNAME = 'MARA' AND FNAME = 'ZEIVR' AND CHANGENR = LV_CHANG.
        IF LV_zeivr IS NOT INITIAL.
          wa_item-zeivr =  LV_zeivr.
          ELSE.
            wa_item-zeivr =  wa_mara-zeivr.
            ENDIF.
********  END AVINASH BHAGAT
*  wa_item-eindt = wa_hdr-eindt.
  SELECT SINGLE eindt slfdt FROM eket
    INTO (wa_item-eindt, wa_item-slfdt)
    WHERE ebeln = wa_hdr-ebeln
      AND ebelp = wa_item-ebelp.

*  GV_SLFDT = wa_item-slfdt.
*  GV_EINDT = wa_item-eindt.

*  CONCATENATE wa_item-eindt+0(4) wa_item-eindt+4(2) wa_item-eindt+6(2)
*INTO GV_EINDT SEPARATED BY '-'.
*
*  CONCATENATE wa_item-slfdt+0(4) wa_item-slfdt+4(2) wa_item-slfdt+6(2)
*INTO GV_SLFDT SEPARATED BY '-'.

*  CONCATENATE GV_EINDT+0(4) GV_EINDT+4(2) GV_EINDT+6(2)
*INTO GV_EINDT SEPARATED BY '-'.

*   CONCATENATE GV_SLFDT+0(4) GV_SLFDT+4(2) GV_SLFDT+6(2)
*INTO GV_SLFDT SEPARATED BY '-'.
...
IF lc EQ 0.
  SELECT * FROM konv INTO TABLE xtkomv
    WHERE knumv = wa_hdr-knumv
      AND kschl IN ('ZPFL', 'ZPFV', 'ZPC1','JEC1','FRA1',
            'FRB1'
...
*    CONCATENATE wa_hdr-zterm_txt INTO wa_hdr-zterm_txt.
  ...
  ENDIF.
ENDIF.
READ TABLE xekpo with key retpo = 'X'.
IF xekpo-retpo = 'X'.
return_no = 'X'.
else.
  return_no = ' '.
...
"WATERMARK LOGIC.
break fujiabap.
*CHECK xekko-frgke = 'X' AND XEKPO-RETPO = 'X'.
IF ( XEKPO-RETPO = 'X' and xekko-frgke = 'X'  ). "SY-SUBRC = 0.
  bg_img = 'ZDRAFT_BG'.
  "'NEW_PO_REJ_PR'. "rej_po
  bg_typ = 'BCOL'.
ELSEIF ( XEKPO-RETPO = 'X' AND ( xekko-frgke = 'R' OR xekko-frgke = '2') ) .
  bg_img = 'REJ_PO_3' .
  bg_typ = 'BCOL'.
 ELSE.
*   bg_img = 'REJ_PO_2' .
*   bg_typ = 'BCOL'.
**   MESSAGE 'CHECK CONDITION' TYPE 'W'.
ENDIF.
...
LOOP AT it_items INTO wa_item.
  ...
  SELECT SINGLE ebeln
          ...
          FROM ekpo
          INTO ls_ekpo
          where ebeln = wa_hdr-ebeln and
                ebelp = wa_item-ebelp and
                knttp = 'E'.
    IF SY-SUBRC = 0 .
      GV_KNTTP = 'E'.
      select single matnr
  ...
  ENDLOOP.
   ...
   CLEAR objname.
    objname = wa_hdr-ebeln.
    PERFORM get_text USING objname 'EKKO' 'F06' 'E' CHANGING wa_hdr-gv_DELIVERy.
     CONDENSE wa_hdr-gv_DELIVERy.

    CLEAR objname.
    objname = wa_hdr-EBELN.
    PERFORM get_text USING objname 'EKKO' 'F21' 'E' CHANGING wa_hdr-gv_PAYment.
