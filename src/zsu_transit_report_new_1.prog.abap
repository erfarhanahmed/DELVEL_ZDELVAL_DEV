...
*&
*&---------------------------------------------------------------------*
REPORT zsu_transit_report_new_1.
...
SELECT-OPTIONS: cust for vbrk-kunag DEFAULT '0000300315' no INTERVALS MODIF ID BU .
...
*IF cust = '0000300315'.
  ...
  PERFORM sort_data.
*ELSE.
*  MESSAGE 'This Report valid For Saudi Customer' TYPE 'S'.
...
**************************added by jyoti on 22.08.2024
****************below logic for performance issue
SELECT a~vbeln
           a~posnr
           a~fkimg
           vgbel
...
*IF it_vbrp IS NOT INITIAL."commented by jyoti on 22.08.2024
...
*         AND bwart = '103'.
**********************ADDED B YJYOTI ON 22.08.2024
**********BELOW LOGIC CHANGE FOR PERFORMANCE ISSUE
SELECT mblnr
           mjahr
           bwart
           matnr
...
******************COMMENTED BY JYOTI ON 22.08.2024
*SELECT mblnr
*       mjahr
*       bwart
*       matnr
...
ENDIF.
*IF it_vapma IS NOT INITIAL."commented b jyoti on 22.08.2024
   IF it_vbrp_vapma IS NOT INITIAL.
  SELECT vbeln
         posnr
         matnr
...
loop at  it_mseg1 INTO DATA(wa).
  DELETE it_mseg where mblnr = wa-smbln.
endloop.
...
READ TABLE it_vbap INTO wa_vbap WITH KEY vbeln = wa_vbrp_vapma-aubel posnr = wa_vbrp_vapma-aupos. "vbeln = wa_vapma-vbeln posnr = wa_vapma-posnr.
IF sy-subrc = 0.
wa_final-kwmeng = wa_vbap-kwmeng.
ENDIF.

READ TABLE it_ekpo INTO wa_ekpo WITH KEY ebeln = wa_vbrp_vapma-bstnk  matnr = wa_vbrp_vapma-matnr ebelp = WA_vbap-posex."ebeln = wa_vapma-bstnk  matnr = wa_vapma-matnr ebelp = wa_vbap-POSEX.
IF sy-subrc = 0.
...
IF sy-subrc = 0.
 wa_final-fkimg  =   wa_final-fkimg - wa_mseg-menge.
...
CLEAR:wa_final,wa_mseg,wa_rev,wa_bkpf,wa_mseg1,wa_vbrp,wa_eikp,wa_mode,wa_kna1,wa_t005t,wa_vbrk,wa_t618t,wa_mara,wa_ekpo,
      wa_vbap,wa_vapma,wa_konv,wa_mseg1,wa_rev,WA_LIKP.
...
FORM get_fcat .
  PERFORM fcat USING :  '1'  'EBELN '           'IT_FINAL'  'Purchase Order No'         '18' ,
                        ...
                        '4'  'WRKST '           'IT_FINAL'  'Old Material Code'         '18' ,
...
*  CONCATENATE p_folder '\' lv_file
  ...
  WRITE: / 'ZSU_TRANSIT REPORT started on', sy-datum, 'at', sy-uzeit.
  OPEN DATASET lv_fullfile
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF sy-subrc = 0.
DATA lv_string_1318 TYPE string.
DATA lv_crlf_1318 TYPE string.
lv_crlf_1318 = cl_abap_char_utilities=>cr_lf.
lv_string_1318 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_1318 lv_crlf_1318 wa_csv INTO lv_string_1318.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_576 TO lv_fullfile.
TRANSFER lv_string_1318 TO lv_fullfile.
