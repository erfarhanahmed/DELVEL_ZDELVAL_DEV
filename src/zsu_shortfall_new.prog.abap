*&---------------------------------------------------------------------*
*& Report ZUS_SHORTFALL_NEW
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZSU_SHORTFALL_NEW.
*
*&Report: ZSU_SHORTFALL_NEW
*&Transaction
...
TYPES: BEGIN OF ty_vbak,
         ...
         abgru  TYPE vbap-abgru, "added by jyoti on 15.04.2024
       END OF ty_vbak,
**********************added b yjyoti on 06.08.2024*****************\
        BEGIN OF ty_vbakp,
          vbeln    TYPE vbak-vbeln,
...
DATA: it_mara    TYPE TABLE OF ty_mara,
      ...
      wa_vbrp    TYPE          ty_vbrp,
       ...
       it_vbrp_new    TYPE TABLE OF ty_vbrp,"added bt jyoti on 22.04.2024
...
DATA : it_vbak  TYPE TABLE OF ty_vbak,
       wa_vbak  TYPE          ty_vbak,
        ...
        it_vbakp   TYPE TABLE OF ty_vbakp,"ADDED BY JYOTI ON 06.08.2024
      wa_vbakp   TYPE ty_vbakp,

      it_jest    TYPE TABLE OF ty_jest,"ADDED BY JYOTI ON 06.08.2024
...
SELECT-OPTIONS : s_vbeln FOR vbak-vbeln,
                 ...
                 p_vkorg FOR vbak-vkorg OBLIGATORY NO INTERVALS DEFAULT 'SU00' MODIF ID bu .
*                   p_matnr TYPE vbap-matnr.
SELECTION-SCREEN: END OF BLOCK b1 .

SELECTION-SCREEN BEGIN OF BLOCK b2 WITH FRAME TITLE TEXT-002 .
PARAMETERS p_down AS CHECKBOX.
PARAMETERS p_folder LIKE rlgrap-filename DEFAULT   '/Delval/Saudi'."Saudi'."Saudi'."Saudi'."saudi'.
      "'/delval/SAUDI'.
...
*  BREAK-POINT.
  SELECT a~vbeln
         ...
         b~kwmeng
         b~werks
         b~abgru "added by jyoti on 15.04.2024
         FROM vbak AS a
         JOIN vbap AS b ON a~vbeln = b~vbeln
*         JOIN vbup AS c ON ( a~vbeln = c~vbeln AND b~posnr = c~posnr ) INTO TABLE it_vbak
         JOIN vbap AS c ON ( a~vbeln = c~vbeln AND b~posnr = c~posnr ) INTO TABLE it_vbak
         WHERE a~vbeln IN s_vbeln
           AND a~audat IN s_date
           AND a~vkorg IN p_vkorg
           AND c~lfsta  NE 'C'
           and c~gbsta  NE 'C'
           AND B~ABGRU not in ('06' , '08', '09')
           and  a~auart not in ( 'SU09', 'SU10','SU15','SU16','SU17',
                          'SU18', 'SU19', 'SU20', 'SU21','SU22' ) .
*         AND b~matnr = p_matnr
*           and c~lfsta  NE 'C'.
*           AND c~absta  NE 'C'.
  " and c~lfgsa  NE 'C'
...
*  DELETE it_vbak WHERE abgru = '06' OR abgru = '08' or  abgru = '09'."added by jyoti on 15.04.2024
*  *************Remove Sales Order for below document type ***********
*  DELETE it_vbak WHERE auart = 'SU09'.
*  DELETE it_vbak WHERE auart = 'SU10'.
*  DELETE it_vbak WHERE auart = 'SU15'.
*  DELETE it_vbak WHERE auart = 'SU16' . "'US05' , 'US06' , 'US09' ,'US10', 'US11', 'US12' ).
*  DELETE it_vbak WHERE auart = 'SU17' .
*  DELETE it_vbak WHERE auart = 'SU18' .
*  DELETE it_vbak WHERE auart = 'SU19' .
*  DELETE it_vbak WHERE auart = 'SU20' .
*  DELETE it_vbak WHERE auart = 'SU21' .
*  DELETE it_vbak WHERE auart = 'SU22' .
...
*    FROM vbak AS a
*         JOIN vbap AS b ON a~vbeln = b~vbeln
*     JOIN  lipsup AS c ON ( c~vgbel = b~vbeln  AND c~vgpos = b~posnr )
*     WHERE a~audat  IN s_date
*      AND a~vkorg IN p_vkorg
**     AND   a~matnr  IN s_matnr
**     AND   a~werks  IN s_werks
*     AND   b~vbeln  IN s_vbeln
*     AND   b~custdeldate IN s_date
*     AND c~gbsta NE 'C'.

******************
*****************ADDED BY JYOTI ON 06.08.2024*********************
*******************LOGIC FOR HOLD AND UNHOLD SALES ORDER
  SELECT vbeln
           posnr_i
           erdat
           ernam
...
LOOP AT it_jest INTO WA_JEST.
  READ TABLE it_vbakp INTO WA_VBAKP WITH KEY OBJNR = WA_JEST-OBJNR.
  IF SY-SUBRC IS INITIAL.
    DELETE it_vbak WHERE VBELN  = WA_VBAKP-VBELN.
  ENDIF.
ENDLOOP.
*************************************************************
  ...
  IF it_vbak IS NOT INITIAL.
    ...
    SELECT vbeln
           ...
           FOR ALL ENTRIES IN it_vbak
           WHERE vbeln = it_vbak-vbeln
           and posnr = it_vbak-posnr "added by jyoti on 15.04.2024
       AND ( abgru NE 6 OR abgru NE 9 ).
 ...
 SELECT vbeln
           ...
           aupos
           matnr
           werks FROM vbrp INTO TABLE it_vbrp_new
           FOR ALL ENTRIES IN it_vbap
           WHERE aubel = it_vbap-vbeln
           and aupos = it_vbap-posnr
           and shkzg NE 'X'.
Data(it_vbrp_new_1) = it_vbrp_new.

REFRESH :it_vbrp_new.
if it_vbrp_new_1 is NOT INITIAL.
 select vbeln from vbrk
   INTO TABLE @data(it_vbrk)
   for ALL ENTRIES IN @it_vbrp_new_1
   where vbeln = @it_vbrp_new_1-vbeln
   and fksto NE 'X'.
endif.
...
*   delete it_vbrp_new WHERE vbeln NE wa_vbrk-vbeln.
* endloop.
...
DELETE IT_MARD WHERE labst EQ 0 AND insme EQ 0 .
...
************************** Collect special indicator vbeln in another table.
  LOOP AT it_mska INTO wa_mska WHERE kalab <> 0.
    ...
    READ TABLE it_vbap INTO DATA(wa_vbap_3) WITH KEY vbeln = wa_mska-vbeln POSNR = wa_mska-POSNR matnr = wa_mska-matnr.
    IF sy-subrc = 0.
      APPEND wa_vbap_3 TO it_vbap3.
    ENDIF.
    DELETE it_vbap  WHERE vbeln = wa_mska-vbeln AND POSNR = wa_mska-POSNR AND matnr = wa_mska-matnr .
  ...
  LOOP AT lt_mara INTO DATA(wa_mara).   """""""""""""""""""""""""""""""""""""Loop according to multiple materials.
    ON CHANGE OF wa_mara-matnr.
      ...
      LOOP AT it_vbap3 INTO wa_vbap3 WHERE matnr = wa_mara-matnr.       """""" 1st proirity to Special indicator stock
        ...
        LOOP AT it_mska INTO wa_mska WHERE vbeln = wa_vbap3-vbeln AND matnr = wa_vbap3-matnr and POSNR = WA_VBAP3-POSNR.
          IF wa_mska-sobkz = 'E'.
        ...
        IF sy-subrc = 0.
          ...
          ON CHANGE OF wa_vbak-matnr.
            ...
            LOOP AT it_mard1 INTO wa_mard1 WHERE matnr = wa_vbak-matnr.
              IF wa_mard1-labst IS NOT INITIAL.   """"""" unresticted stock against same material
            ...
            ENDLOOP.
*            STOCK1 = stock.
            IF STOCK IS INITIAL. "ADDED BY JYOTI O N 02.08.2024
            CLEAR:lv_mska_stock .
            LOOP AT it_mska INTO wa_mska WHERE kalab <> 0  AND matnr = wa_vbak-matnr AND VBELN = wa_vbap3-vbeln AND"ADDED BY JYOTI O N 02.08.2024
                                                             posnr = wa_vbap3-posnr..
              lv_mska_stock =  lv_mska_stock + wa_mska-kalab.   " Special indicator stock"ADDED BY JYOTI O N 02.08.2024
            ENDLOOP.
            ELSE.
              CLEAR:lv_mska_stock .
            LOOP AT it_mska INTO wa_mska WHERE kalab <> 0  AND matnr = wa_vbak-matnr.
              lv_mska_stock =  lv_mska_stock + wa_mska-kalab.   " Special indicator stock
            ENDLOOP.
            ENDIF.
            IF lv_mska_stock IS NOT INITIAL.
              stock = stock + lv_mska_stock. "both unresticted stock and special indicator stock
              mb52 = stock.
              lv_stock = stock.
            ENDIF.

**************************transit quantity on material
            READ TABLE it_trans INTO wa_trans WITH KEY matnr = wa_vbak-matnr ."posnr =  wa_vbak-posnr.
            IF sy-subrc = 0.
...
*        ENDLOOP.
*******************************************added by jyoti on 22.04.2024**************************
*******************************delivery quantity against sales order if delivery done.
          lOOP AT it_vbrp_new INTO wa_vbrp_new WHERE aubel = wa_vbak-vbeln AND aupos = wa_vbak-posnr.
          wa_final-lfimg = wa_final-lfimg + wa_vbrp_new-fkimg .
        ENDLOOP.
***************************************************************************************************

        wa_final-pnd_so = wa_vbak-kwmeng - wa_final-lfimg. "pending so depends on so quantity minus delivery qunatity

        wa_vbak-kwmeng = wa_final-pnd_so.   "move pending so in so quantity
*******************if matnr is not change and stock is initial and special indiator stock is available then
*************special stock indicator is move on stock variable.
        if stock is INITIAL.
          stock =  wa_final-kalab.
        endif.

***        IF STOCK GE WA_VBAP3-KWMENG .
***********************ADDED BY JYOTI ON 31.07.2024
************ LOGIC FOR SPECIAL INDICATOR + UNRESCTICTED STOCK FOR SAME MATERIAL.
   IF STOCK IS NOT INITIAL.
     IF  STOCK GT WA_FINAL-ALLOC. "IF STOCK IS GRATER THAN ALLOCATION(SPECIAL STOCK INDICATOR 'E')
        IF STOCK LE wa_final-pnd_so. " IF STOCK LESS THAN PENDING SO
          wa_final-alloc = STOCK. " if both special stock and unresticted stock against same material
          stock = stock -  wa_final-alloc.  "if allocation then check remaining stock
          wa_final-so_srt  = wa_final-pnd_so - wa_final-alloc. "shortfall qunatity depends on allocation
          qty = wa_final-alloc."alloctaion details
       else.     " STOCK IS GREATER THAN PENDING SO
           IF wa_final-pnd_so GT wa_final-alloc. "IF PENDING SO IS GREATER THAN ALLOCATION THEN PENDING SO IS ALLOCATED TO ALLOCATION
             wa_final-alloc = wa_final-pnd_so."wa_final-alloc. "if special stock  against same material
             stock = stock -  wa_final-alloc. "if allocation then check remaining stock
             wa_final-so_srt  = wa_final-pnd_so - wa_final-alloc."shortfall qunatity depends on allocation
             qty = wa_final-alloc. "alloctaion details
           ELSE.
             wa_final-alloc = wa_final-alloc." wa_final-pnd_so."wa_final-alloc. "if special stock  against same material
             stock = stock -   wa_final-alloc. "if allocation then check remaining stock
             wa_final-so_srt  = wa_final-pnd_so - wa_final-alloc."shortfall qunatity depends on allocation
             qty = wa_final-alloc. "alloctaion details
           ENDIF.
        ENDIF.
******************************************************************************
     ELSE. "IF  STOCK LESS THAN ALLOCATION (SPECIAL STOCK INDICATOR 'E')
        wa_final-alloc = wa_final-alloc. "if special stock  against same material
        stock = stock -   wa_final-alloc. "if allocation then check remaining stock
        wa_final-so_srt  = wa_final-pnd_so - wa_final-alloc."shortfall qunatity depends on allocation
        qty = wa_final-alloc."alloctaion details
     ENDIF.
  else.
      wa_final-alloc = wa_final-alloc.
      tran_st = tran_st - wa_final-alloc."Check remaing trnaist quantity on shortfall qunatity
      wa_final-so_srt = wa_final-pnd_so - wa_final-alloc. "shortfall qunatity depends on allocation
       qty = wa_final-alloc."alloctaion details
  ENDIF.
*****************************alllocation detaails concatenate with allocation if special stock
...
******when stock is completed used then after transit qunatity assigned to stock
        IF stock = 0 AND tran_st NE 0. "IF STOCK IS ZERO AND VALUE IN TRANSIT QUANTITY
          if wa_final-alloc GT WA_FINAL-PND_SO."ADDED BY JYOTI ON 25.07.2024 " IF ALLOCATION  GRATER THAN PENDING SO
          stock = tran_st."if transit quantity is available then transit qty assign to stock
          wa_final-alloc = wa_final-alloc + wa_final-so_srt ."shortfall qunatity added in  allocation
          wa_final-so_srt = wa_final-pnd_so - wa_final-alloc. "shortfall qunatity depends on allocation
          tran_st = tran_st - wa_final-so_srt."Check remaing trnaist quantity on shortfall qunatity
          qty = wa_final-alloc."alloctaion details
          CONCATENATE  'T' qty INTO t SEPARATED BY '-'. " transit quantity is allpcated to so then in allocation details display as T-qty
          CONDENSE t.
          ENDIF.  "ADDED BY JYOTI ON 25.07.2024
        ENDIF.
...
********* If stock is zero then allocate transit stock.
******when stock is completed used then after transit qunatity assigned to stock
***********************commented by jyoti on 07.08.2024 ffor testing purpose***********************************************************
*          IF stock = 0 AND tran_st NE 0. "IF STOCK IS ZERO AND VALUE IN TRANSIT QUANTITY
*          if wa_final-alloc lT WA_FINAL-PND_SO."ADDED BY JYOTI ON 25.07.2024 " IF ALLOCATION LESS THAN PENDING SO
*           wa_final-alloc = wa_final-alloc + wa_final-so_srt ."shortfall qunatity added in  allocation
*          tran_st = tran_st - wa_final-so_srt."Check remaing trnaist quantity on shortfall qunatity
*          qty = wa_final-so_srt."alloctaion details
*          wa_final-so_srt = wa_final-pnd_so - wa_final-alloc. "shortfall qunatity depends on allocation
*          CONCATENATE  'T' qty INTO t SEPARATED BY '-'. " transit quantity is allpcated to so then in allocation details display as T-qty
*          CONDENSE t.
*
...
***********************************************************************************************
        ...
        CLEAR :alloc,qty_al."clear vairables


        "        CLEAR:S,T.
        alloc = wa_final-alloc."allocation assign to varialble
        IF wa_final-alloc IS NOT INITIAL." check allocation is available or not
          IF mb52 GE alloc."check mb52 stock is greater than equal to allocation.
            CLEAR:qty.
            qty = alloc.
            qty_al = alloc.
***            CONCATENATE 'S' QTY INTO S SEPARATED BY '-'.
***            CONDENSE S.
            alloc = mb52 - alloc."allocation minus from stock mb52
*    IF alloc > 0.
            mb52 = mb52 - qty_al."allocation minus from stock
*    ENDIF.
...
****************************************  end 22.01.2024
      ...
      LOOP AT it_vbap INTO wa_vbap WHERE matnr = wa_mara-matnr.
        ...
        READ TABLE it_vbak INTO wa_vbak WITH KEY vbeln = wa_vbap-vbeln matnr = wa_vbap-matnr posnr = wa_vbap-posnr.
        IF sy-subrc = 0.
...
*==============================================================
          ...
          ON CHANGE OF wa_vbak-matnr.
            IF lv_stock  = 0. """"""""""""""""""""""""""""""""""""""""""""""  ADDED IF STOCK IS NOT AVAILABLE IN ABOVE LOOP by Shubhangi Sarode
              ...
              ENDIF.
            ENDIF.      """"""""""""""""""""""""""""""""""""""""""""""""""""
            if tran_st is INITIAL."added by jyoti on 05.08.2024
            READ TABLE it_trans INTO wa_trans WITH KEY matnr = wa_vbak-matnr." vbeln = wa_vbap-vbeln posnr = wa_vbap-posnr.
            IF sy-subrc = 0.
              tran_st = wa_trans-fkimg.
            ENDIF.
            endif.
...
*          wa_final-lfimg = wa_final-lfimg + wa_lips-lfimg .
*        ENDLOOP.
        LOOP AT it_vbrp_new INTO wa_vbrp_new WHERE aubel = wa_vbap-vbeln AND aupos = wa_vbap-posnr.
            wa_final-lfimg = wa_final-lfimg + wa_vbrp_new-fkimg.
        ENDLOOP.

        wa_final-pnd_so = wa_vbak-kwmeng - wa_final-lfimg."commented on 15.04.2024 added by jyoti
...
***        BREAK primus.
        ...
        ELSEIF stock LE wa_vbap-kwmeng .
          if stock LT wa_final-pnd_so.
          wa_final-alloc = stock .
          else.
            wa_final-alloc = wa_final-pnd_so ."added by jyoti on 23.04.2024
          endif.
...
*          if  stock LT wa_final-pnd_so. "added by jyoti omn 23.04.2024 "commented by jyoti on 24.04.2024
...
**          tot_alloc_stk = allocation + stk - mb52.
*          tot_alloc_stk = stk - stock."commented by jyoti on 23.04.2024

          tot_alloc_stk = wa_final-alloc + stk - mb52."added by jyoti on 23.04.2024
          CONDENSE tot_alloc_stk.
           if tot_alloc_stk  = 0.
             CLEAR tot_alloc_stk .
          endif.
          if tot_alloc_stk IS NOT INITIAL.
        ...
        IF it_final IS NOT INITIAL.
          LOOP AT it_final INTO wa_final WHERE matnr = wa_mara-matnr AND so_srt IS NOT INITIAL ."and KWMENG le stock.
            ...
            IF stock < wa_final-kwmeng.
              ...
              ENDIF.
            ELSE.
             if  stock LT  wa_final-so_srt."added by jyoti on 23.04.2024
...
******************************************************************** end on dt 01.02.2024
  ...
  ENDLOOP.
*  SORT it_final ASCENDING BY matnr custdeldate. "vbeln posnr.
   SORT it_final ASCENDING BY matnr custdeldate vbeln POSNR.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  lv_file = 'ZSU_SHORTFALL_TRANSIT.TXT'.


  CONCATENATE p_folder '/' sy-datum sy-uzeit lv_file
    INTO lv_fullfile.

  WRITE: / 'ZSU_SHORTFALL REPORT started on', sy-datum, 'at', sy-uzeit.
  OPEN DATASET lv_fullfile
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF sy-subrc = 0.
DATA lv_string_2179 TYPE string.
DATA lv_crlf_2179 TYPE string.
lv_crlf_2179 = cl_abap_char_utilities=>cr_lf.
lv_string_2179 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_2179 lv_crlf_2179 wa_csv INTO lv_string_2179.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_1008 TO lv_fullfile.
*TRANSFER lv_string_1008 TO lv_fullfile.
TRANSFER lv_string_2179 TO lv_fullfile.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET lv_fullfile
    FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF sy-subrc = 0.
DATA lv_string_2216 TYPE string.
DATA lv_crlf_2216 TYPE string.
lv_crlf_2216 = cl_abap_char_utilities=>cr_lf.
lv_string_2216 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_2216 lv_crlf_2216 wa_csv INTO lv_string_2216.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_1008 TO lv_fullfile.
*TRANSFER lv_string_1008 TO lv_fullfile.
TRANSFER lv_string_2216 TO lv_fullfile.
    CONCATENATE 'File' lv_fullfile 'downloaded' INTO lv_msg SEPARATED BY space.
    MESSAGE lv_msg TYPE 'S'.
  ENDIF.
ENDFORM.
...
*           WHERE vbeln = it_vbrk-vbeln.
    ...
    SELECT a~vbeln
       ...
       INTO TABLE it_vbrp_vapma
          ...
          JOIN vapma as b
          on ( b~vbeln = a~aubel
          AND b~posnr = a~aupos ) "and a~matnr = b~matnr and a~werks = b~werks )
         FOR ALL ENTRIES IN it_vbrk
...
*  IF it_vbrp IS NOT INITIAL."commented by jyoti on 23.07.2024
...
**************************************************************
    ...
    SELECT mblnr
           ...
           vbeln_im
           vbelp_im
           menge FROM mseg INTO TABLE it_mseg
           FOR ALL ENTRIES IN it_vbrp_vapma
...
*****************************commented by jyotio n 23.07.2024
*           FOR ALL ENTRIES IN it_vbrp
...
*           AND bwart = '105' .
  ...
  ENDIF.
*  IF it_vapma IS NOT INITIAL."commented b jyoti on 23.07.2024
   IF it_vbrp_vapma IS NOT INITIAL.
    SELECT vbeln
           posnr
           matnr
...
*           FOR ALL ENTRIES IN it_vapma  "ommented by jyoti on 23.07.2024
...
*  BREAK PRIMUSUSA.
    ...
    SELECT mblnr
           ...
           vbeln_im
           vbelp_im
           menge FROM mseg INTO TABLE it_mseg1
           FOR ALL ENTRIES IN it_mseg
...
*            AND BUKRS = 'US00'. "added by jyoti on 23.07.2024
            AND BUKRS = 'SU00'. "added by jyoti on 23.07.2024
*         AND blart = 'TR'.
...
******************commented by jyoti on 23.07.2024****************
*    SELECT vbeln
*           verur FROM likp INTO TABLE it_likp
...
*    READ TABLE it_likp INTO wa_likp INDEX 1.
    ...
    READ TABLE it_vbrk INTO wa_vbrk WITH KEY vbeln = wa_vbrp_vapma-vbeln."wa_vbrp-vbeln.
    IF sy-subrc = 0.
    ...
    READ TABLE it_konv INTO wa_konv WITH KEY knumv = wa_vbrk-knumv kposn = wa_vbrp_vapma-posnr. "wa_vbrp-posnr.
    IF sy-subrc = 0.
...
*     READ TABLE it_mseg INTO wa_mseg WITH KEY xblnr_mkpf = wa_vbrp_vapma-vgbel."

READ TABLE it_mseg INTO wa_mseg WITH KEY VBELN_IM = wa_likp-vbeln
                                         VBELP_IM = wa_vbrp_vapma-vgpos.

    IF sy-subrc = 0.
         wa_transit-fkimg    =   wa_transit-fkimg - wa_mseg-menge.
...
*     READ TABLE it_rev INTO wa_rev WITH KEY xblnr_mkpf = wa_vbrp_vapma-vgbel."
*    IF sy-subrc = 4.
       IF wa_likp-trspg NE '03'.
         if wa_transit-fkimg <> 0.
...
*    ENDIF.
  ...
  LOOP AT it_marc INTO wa_marc.
    ...
    LOOP AT it_transit INTO wa_transit WHERE matnr = wa_marc-matnr.
      ...
      WA_TRANS-VBELN = wa_transit-VBELN.
      WA_TRANS-POSNR = wa_transit-POSNR.
    ENDLOOP.
