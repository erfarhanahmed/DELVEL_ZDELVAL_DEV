...
*Internal Table for sorting
 ...
 TYPES: BEGIN OF idata ,
          ...
          debit1      LIKE bsik-dmbtr,         "Amount in local curr.
...
******************************************************************
          ...
          dmbtr1      TYPE dmbtr,
          tcs_amt     TYPE dmbtr,
          debit2      TYPE  bsik-dmbtr,
          debit3      TYPE  bsik-dmbtr,
        END OF idata.
...
******************SELECTION SCREEN
 ...
 PARAMETERS: plant LIKE bsik-bukrs  OBLIGATORY DEFAULT '1000'.
...
*----------------------------------------------------------------------*
 FORM datalist_bsik .
   ...
   DATA:
     ...
     it_bseg_1  TYPE TABLE OF ty_bseg,
...
*     BREAK primusabap.
     SELECT bukrs
       ...
       AND   ktosl EQ  'WIT'
       AND   qsskz NOT IN ('4Q','5Q') ."OR QSSKZ <> '5Q' .
...
**************************Added by komal.s on 21-1-2025****************
    select bukrs,
            ...
            umskz,
            shkzg
       FROM bseg into table @data(it_bseg_4)
                 FOR ALL ENTRIES IN @itab
                 where rebzg = @itab-belnr
                 and   bukrs = @itab-bukrs
                 AND   gjahr = @itab-gjahr.
  if it_bseg_4 is NOT INITIAL.
      select bukrs,
            belnr,
            gjahr,
            buzei,
            ...
            umskz,
            shkzg
       FROM bseg into table @data(it_bseg_rev)
                 FOR ALL ENTRIES IN @it_bseg_4
                 where belnr = @it_bseg_4-belnr
                 and   bukrs = @it_bseg_4-bukrs
                 AND   gjahr = @it_bseg_4-gjahr
                 and shkzg = 'H'.
    endif.
**************************Ended  by komal.s on 21-1-2025****************
     ...
     SELECT bukrs,
            ...
            shkzg
       FROM bseg
       INTO TABLE @DATA(it_bseg_new1)
       FOR ALL ENTRIES IN @it_bseg_new
       WHERE REBZG = @it_bseg_new-belnr
       AND   bukrs = @it_bseg_new-bukrs
       AND   gjahr = @it_bseg_new-gjahr.
*       AND   UMSKZ EQ 'A'.
     ...
     select bukrs,
            ...
            rebzg,
            umskz  from bseg
                   into table @data(it_bseg_new2)
                   FOR ALL ENTRIES IN @it_bseg_new1
                   where belnr = @it_bseg_new1-belnr
                   AND   bukrs = @it_bseg_new1-bukrs
                   AND   gjahr = @it_bseg_new1-gjahr
                   AND   UMSKZ EQ 'A'.
...
* ADDED LOGIC FOR VENDOR REFERENCE DOCUMENT NOT SHOWING
     ...
     SELECT * FROM tvarvc INTO TABLE @DATA(lt_tvarv) WHERE name EQ 'ZFI_VEN_AGEING'.
       ...
       LOOP AT lt_data INTO ls_data FROM lv_index.
         IF ls_data-rebzg = itab-belnr AND ls_data-rebzj = itab-gjahr AND ls_data-rebzz = itab-buzei.
           IF ls_data-shkzg = 'H'.
             ...
             IF ls_data-blart NE 'KZ'.
               IF ls_data-umskz IS INITIAL.
                 itab-debit1 = itab-debit1 - ls_data-dmbtr.
               ENDIF.
             ELSEIF ls_data-blart EQ 'KZ'.
               itab-debit3 = itab-debit3 - ls_data-dmbtr.
...
*                endif.
     ...
     ENDIF.
*      itab-debit =  itab-debit1.
     MODIFY itab TRANSPORTING debit debit1 debit3.
...
*        ENDIF.
     ...
     IF itab-umskz  = ''.
       itab-group  = 'Normal'.
*       itab-debit1 = itab-debit1 + itab-dmbtr.
     ELSE.
...
*     READ TABLE lt_data INTO ls_data
     ...
     READ TABLE it_bseg INTO wa_bseg WITH KEY belnr = itab-belnr.
     IF sy-subrc = 0.
       itab-dmbtr1 = wa_bseg-dmbtr.
     ENDIF.
     CLEAR wa_bseg-dmbtr.
*     BREAK primusabap.
     READ TABLE it_bseg_1 INTO wa_bseg_1 WITH KEY belnr = itab-belnr.
     IF sy-subrc = 0.
       itab-tcs_amt = wa_bseg_1-dmbtr.
     ENDIF.
     CLEAR wa_bseg_1-dmbtr.
******************************komal.s on 20.01.2024**************************
*     READ TABLE it_bseg_new1 INTO DATA(wa_bseg_new1) WITH KEY rebzg = itab-belnr.
     loop at it_bseg_new1 into DATA(wa_bseg_new1) where  rebzg = itab-belnr.
*     IF sy-subrc = 0.
       loop at it_bseg_new2 into data(wa_bseg_new2) where belnr = wa_bseg_new1-belnr.
       itab-debit2 =  itab-debit2 + wa_bseg_new2-dmbtr.
       clear wa_bseg_new2.
       endloop.

       MODIFY ITAB TRANSPORTING debit2.
       clear wa_bseg_new1.
      endloop.
      if itab-debit1 is NOT INITIAL.
      itab-debit1 =  itab-debit1 - itab-debit2.
      endif.
     loop at it_bseg_4 into data(wa_bseg_4) where rebzg = itab-belnr.
       loop at it_bkpf into data(wa_bkpf) where belnr = wa_bseg_4-belnr.
*        if sy-subrc = 0.
          if wa_bseg_4-umskz is INITIAL.
*             wa_bkpf-blart ne 'KZ'.
            if wa_bkpf-blart Eq 'KZ'.
              if itab-debit3 is NOT INITIAL.
*             if itab-debit3 lt '0'.
*            itab-netbal = itab-netbal - itab-debit3.
*            else.
*               itab-netbal = itab-netbal + itab-debit3.
*            endif.
             itab-debit3 = itab-debit3 - itab-debit3
             .
             if itab-credit ne ( itab-netbal * -1 ).
               itab-netbal = itab-netbal - wa_bseg_4-dmbtr.
            endif.
              endif.
          else.
            if itab-debit1 is NOT INITIAL.
*              if itab-debit1 lt '0'.
*            itab-netbal = itab-netbal - itab-debit1.
*            else.
*               itab-netbal = itab-netbal + itab-debit1.
*            endif.
            itab-debit1 = itab-debit1 - itab-debit1.
           if itab-credit ne ( itab-netbal * -1 ).
          itab-netbal = itab-netbal - wa_bseg_4-dmbtr.
           endif.
          endif.
...
*          elseif wa_bseg_4-umskz is INITIAL and wa_bkpf-blart Eq 'KZ'.
*          itab-debit3 = itab-debit3 - itab-debit3.
*           itab-netbal = itab-netbal - wa_bseg_4-dmbtr.
        endif.
*        endif.
...
*         loop at it_bseg_4 into data(wa_bseg_4) where rebzg = itab-belnr
       ...
       loop at it_bseg_rev into data(wa_bseg_rev) where belnr = wa_bseg_4-belnr
                                                 and umskz = 'A'.
         loop at it_bkpf into data(wa_bkpf_1) where belnr = wa_bseg_rev-belnr.
*          if sy-subrc = 0.
          if wa_bseg_rev-umskz = 'A'.
            if itab-debit2 is NOT INITIAL.
*               if itab-debit2 lt '0'.
*            itab-netbal = itab-netbal - itab-debit2.
*            else.
*               itab-netbal = itab-netbal + itab-debit2.
*            endif.


            itab-debit2 = itab-debit2 - itab-debit2.
             if itab-credit ne ( itab-netbal * -1 ).
            itab-netbal = itab-netbal - wa_bseg_rev-dmbtr.
            endif.
         endif.
         endif.
          endloop.
         endloop.
         clear : wa_bseg_4, wa_bseg_rev.
*      endloop.

      endloop.


********************komals. on 20.01.2024*****************************
...
*          itab-debit1 = itab-debit1 + ls_data-dmbtr.
   ...
   DELETE itab WHERE augbl ne space.
   REFRESH lt_bkpf_new.
...
*----------------------------------------------------------------------*
 FORM fill_fieldcatalog.
   ...
   PERFORM f_fieldcatalog USING '3'   'GROUP'     'GL Type'.
*   PERFORM f_fieldcatalog USING '2'   'LIFNR'     'Vendor code '.
   PERFORM f_fieldcatalog USING '4'   'XBLNR'     'Vendor Invoice No.'.
   PERFORM f_fieldcatalog USING '5'   'BELNR'     'Doc No.'.
   PERFORM f_fieldcatalog USING '6'   'BLART'     'Doc Type'.
   PERFORM f_fieldcatalog USING '7'   'BUDAT'     'Posting Date'.
   PERFORM f_fieldcatalog USING '8'   'BLDAT'     'Doc Date'.
   PERFORM f_fieldcatalog USING '9'   'DUEDATE'   'Due Date'.
   PERFORM f_fieldcatalog USING '10'   'GRN_DT'    'GRN Date'.
   PERFORM f_fieldcatalog USING '11'   'VTEXT'    'Payment Terms'.
   PERFORM f_fieldcatalog USING '12'   'CURR'     'Amt Document Currency'.
   PERFORM f_fieldcatalog USING '13'   'WAERS'    'Currency Key'.
   PERFORM f_fieldcatalog USING '14'   'CREDIT'    'Invoice Amt.'.
   PERFORM f_fieldcatalog USING '15'   'DEBIT1'    'Debit Memo Amt'.
   PERFORM f_fieldcatalog USING '16'  'NETBAL'    'Net Pending'.
   PERFORM f_fieldcatalog USING '17'  'NETB30'    rc13. "'Net Balance'.
   PERFORM f_fieldcatalog USING '18'  'NETB60'    rc14. "'Net Balance'.
   PERFORM f_fieldcatalog USING '19'  'NETB90'    rc15. "'Net Balance'.
   PERFORM f_fieldcatalog USING '20'  'NETB120'   rc16. "'Net Balance'.
   PERFORM f_fieldcatalog USING '21'  'NETB180'   rc17. "'Net Balance'.
   PERFORM f_fieldcatalog USING '22'  'NETB360'   rc18. "'Net Balance'.
   PERFORM f_fieldcatalog USING '23'  'NOT_DUE'   'Not Due'. "'Net Balance'.
*   PERFORM f_fieldcatalog USING '23'  'NETB_N30'   rc19. "'Net Balance'.
   PERFORM f_fieldcatalog USING '24'  'NETB_N60'   rc20. "'Net Balance'.
   PERFORM f_fieldcatalog USING '25'  'NETB_N90'   rc21. "'Net Balance'.
   PERFORM f_fieldcatalog USING '26'  'NETB_N120'  rc22. "'Net Balance'.
   PERFORM f_fieldcatalog USING '27'  'NETB_N180'  rc23. "'Net Balance'.
   PERFORM f_fieldcatalog USING '28'  'NETB_N360'  rc24. "'Net Balance'.
   PERFORM f_fieldcatalog USING '29'  'DAY'       'Over Due Days'.
   PERFORM f_fieldcatalog USING '30'  'PDC'       'PDC Amt.'.
   PERFORM f_fieldcatalog USING '31'  'INVOICE'       'Invoice No.'.
   PERFORM f_fieldcatalog USING '32'  'STATUS'       'MSME Status'.
   PERFORM f_fieldcatalog USING '33'  'DMBTR1'        'TDS Amt.'.
   PERFORM f_fieldcatalog USING '34'  'TCS_AMT'        'TCS Amt.'.
   PERFORM f_fieldcatalog USING '35'  'DEBIT2'        'Advance Amt.'.
   PERFORM f_fieldcatalog USING '35'  'DEBIT3'        'Payment Amt.'.
...
*  Title
   wa_header-typ  = 'H'.
   wa_header-info = 'Vendor Ageing Report'.
...
*  t_fieldcat-decimals_out = '2'.
   ...
   IF f1 = 'DEBIT1'    OR f1 = 'CREDIT'    OR f1 = 'DEBIT30'  OR f1 = 'CREDIT30'  OR
...
****************************ADDED BY JYOTI ON 15.01.2024********
     ...
     ls_final-debit2           = abs( itab-debit2 ).
     IF itab-debit2 < 0.
       CONDENSE ls_final-debit2.
       CONCATENATE '-' ls_final-debit2 INTO ls_final-debit2.
     ENDIF.

     ls_final-debit3           = abs( itab-debit3 ).
     IF itab-debit3 < 0.
       CONDENSE ls_final-debit3.
       CONCATENATE '-' ls_final-debit3 INTO ls_final-debit3.
     ENDIF.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
   ...
   IF sy-subrc = 0.
DATA lv_string_2076 TYPE string.
DATA lv_crlf_2076 TYPE string.
lv_crlf_2076 = cl_abap_char_utilities=>cr_lf.
lv_string_2076 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_2076 lv_crlf_2076 wa_csv INTO lv_string_2076.
  CLEAR: wa_csv.
ENDLOOP.
