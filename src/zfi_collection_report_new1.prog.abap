...
*AND BELNR = @IT_BSEG-BELNR AND KUNNR = @IT_BSEG-KUNNR
      ...
      SELECT  belnr , blart , budat, augbl, wrbtr , xblnr , dmbtr,GJAHR, ZUONR FROM bsad INTO TABLE @DATA(it_bsad_doc)
        FOR ALL ENTRIES IN @it_bseg
...
*                     WHERE AUGBL =  @IT_BSEG-AUGBL  AND AUGDT = @IT_BSEG-AUGDT AND blart ne 'RV' "BLART NOT IN @DOCTYP[] "need to uncomment logic for cust 201179
      ...
      IF NOT it_bseg1 IS INITIAL.
        SELECT  belnr,
               ...
               gjahr = @it_bseg1-gjahr(4) %_HINTS DB6 'INDEX("BSAD" "BSAD~1")'..
*        AND  augbl = @wa_bseg-augbl AND augdt = @wa_bseg-augdt %_HINTS DB6 'INDEX("BSAD" "BSAD~1")'..
        ...
        SELECT  belnr,
           ...
           FOR ALL ENTRIES IN @it_bseg1 WHERE
           belnr = @it_bseg1-belnr and
            gjahr = @it_bseg1-gjahr(4) %_HINTS DB6 'INDEX("BSAK" "BSAK~1")'.
      ...
      SELECT  belnr,blart,budat,rebzj, rebzg, shkzg, wrbtr, kunnr, dmbtr, buzei FROM bsid INTO TABLE @DATA(it_bsid)
        FOR ALL ENTRIES IN @it_bseg
    ...
    if it_bsid is NOT INITIAL.
      ...
      select       belnr,
              ...
              FOR ALL ENTRIES IN @it_bsid
              WHERE augbl = @it_bsid-belnr
             and gjahr = @it_bsid-rebzj.

            DELETE it_bseg2 where kostl is NOT INITIAL.


          loop at it_bsid INTO data(wa) .
            DELETE it_bseg2 where belnr = wa-belnr and buzei = wa-buzei.
            DELETE it_bseg3 where augbl = wa-belnr and buzei = wa-buzei.
           endloop.

           DELETE it_bseg2 where augbl is NOT INITIAL.
        endif.
...
*            IF it_bsad_doc IS NOT INITIAL.
              READ TABLE it_bsad_doc INTO DATA(wa_bsad_doc) WITH KEY ZUONR = WA_BSEG-VBELN augbl = wa_bseg-augbl ." augdt = ls_bseg-augdt..

              IF sy-subrc = 0 .
                <wa_final>-pay_doc_typ   =  wa_bsad_doc-blart.
                <wa_final>-pay_doc_no    =  wa_bsad_doc-belnr.
                  <wa_final>-date_coll     =  wa_bsad_doc-budat.
*                  <wa_final>-other_amt_coll = WA_bsad_doc-dmbtr."added by jyotio n 18.11.2024
*                  <wa_final>-ld_date_coll = WA_bsad_doc-budat."added by jyotio n 18.11.2024
...
*                    <wa_final>-other_amt_coll = <WA_bsad>-dmbtr."added by jyotio n 18.11.2024
*                      <wa_final>-ld_date_coll = <WA_bsad>-budat."added by jyotio n 18.11.2024
              ENDIF.
*            ENDIF.
...
**
            ...
            READ TABLE it_bsid INTO DATA(wa_bsid3) WITH KEY belnr =  wa_bsid2-belnr  rebzj = wa_bkpf-gjahr..
*            READ TABLE it_bsid INTO DATA(wa_bsid3) WITH KEY rebzg =  wa_bkpf-belnr  rebzj = wa_bkpf-gjahr..
            IF sy-subrc = 0.
              READ TABLE it_bseg2 INTO data(wa_bseg2) with key belnr = wa_bsid3-belnr  gjahr = wa_bsid2-rebzj.
               iF sy-subrc = 0.
...
*            if WA_BSID-WRBTR is NOT INITIAL.
            ...
            IF  gv_days LE '90'.
              IF wa_bsid2-shkzg = 'H'.
*                ls_col2    = wa_bsid2-dmbtr * -1."commented byjyoti on 19.11.2024
                ls_col2    = wa_bseg2-dmbtr * -1.

              ELSE.
*                ls_col3     = wa_bsid2-dmbtr . commented byjyoti on 19.11.2024
                ls_col3    = wa_bseg2-dmbtr * -1.
              ENDIF.
              <wa_final>-amt_coll   = <wa_final>-amt_coll + ls_col2 + ls_col3 .
              gv_days1 =  gv_days.
                <wa_final>-gv_days1 = gv_days1.
...
*            endif.
            ...
            CLEAR: wa_bsid, ls_col2 , ls_col3,wa_bsid, wa_bseg2,wa_bsid3.
          ENDLOOP.
          <wa_final>-amt_coll0   = <wa_final>-amt_coll0 + <wa_final>-amt_coll + <wa_final>-amt_coll1 + <wa_final>-amt_coll2 + <wa_final>-amt_coll3 + <wa_final>-other_amt_coll.
          <wa_final>-bal_any = <wa_final>-inv_amt +  <wa_final>-amt_coll0.
        ENDIF.
...
*          LOOP AT it_bsad_ld INTO DATA(ls_bsad_ld) WHERE BELNR = wa_bkpf-xblnr and augbl = wa_bseg-augbl AND augdt = wa_bseg-augdt .
          LOOP AT it_bsad_ld INTO DATA(ls_bsad_ld) WHERE augbl = <WA_FINAL>-PAY_DOC_NO.
...
*
          ...
          if sy-subrc is NOT INITIAL.
           ...
           loop at it_bseg1 INTO data(wa_bseg1) where belnr = wa_bseg-augbl.
               CASE wa_bseg1-shkzg.
                 WHEN 'H'.
                ls_bsad_ld-dmbtr = wa_bseg1-dmbtr * -1.
              WHEN 'S'.
                ls_bsad_ld-dmbtr = wa_bseg1-dmbtr.
            ENDCASE.


             <wa_final>-ld_date_coll =  <wa_final>-clear_date.
             <wa_final>-other_amt_coll = ls_bsad_ld-dmbtr + wa_final-other_amt_coll .
            if  <wa_final>-bal_any is NOT INITIAL."added by jyoti on17.10.2024
*              if wa_bseg1-shkzg = 'H'.
            <wa_final>-amt_coll0 = wa_final-amt_coll0 - ls_bsad_ld-dmbtr .
...
*          endloop.
      ...
      if sy-subrc is NOT INITIAL.
        loop at it_bsid INTO wa_bsid WHERE rebzg =  wa_bkpf-belnr AND rebzj = wa_bkpf-gjahr.
          loop at it_bseg1 INTO wa_bseg1 where belnr = wa_bsid-belnr.
...
*          loop at it_bsid INTO wa_bsid WHERE rebzg =  wa_bkpf-belnr AND rebzj = wa_bkpf-gjahr.
          loop at it_bseg3 INTO data(wa_bseg3) WHERE augbl = wa_bsid-belnr.
             CASE wa_bseg3-shkzg.
               WHEN 'H'.
                ls_bsad_ld-dmbtr = wa_bseg3-dmbtr * -1.
               WHEN 'S'.
                ls_bsad_ld-dmbtr = wa_bseg3-dmbtr.
               ENDCASE.
             wa_final-ld_date_coll =  <wa_final>-clear_date.
             wa_final-other_amt_coll = ls_bsad_ld-dmbtr + wa_final-other_amt_coll .
         ...
         if wa_final-other_amt_coll gt 0.
                wa_final-other_amt_coll =  wa_final-other_amt_coll * -1.
             endif..


         if <wa_final>-bal_any is NOT INITIAL.
           <wa_final>-bal_any = <wa_final>-bal_any + wa_final-other_amt_coll.
          endif.
...
*        IF  gv_days1 GT 0 AND gv_days1 LE 90. "Added by jyoti on 16.10.2024
         if ls_final1-other_amt_coll is NOT INITIAL.
           ls_final1-amt_coll = ls_final1-amt_coll + ls_final1-other_amt_coll.
         endif.
...
*          ls_final1-due_buk_90 = ls_final1-amt_coll.
          ls_final1-due_buk_90 = ls_final1-amt_coll  + ls_final1-amt_coll1 + ls_final1-amt_coll2 + ls_final1-amt_coll3.
...
*          ls_final1-due_buk_90 =  ls_final1-due_buk_90 + ls_final1-amt_coll.
          ls_final1-due_buk_90 =  ls_final1-due_buk_90 + ls_final1-amt_coll + ls_final1-amt_coll1 + ls_final1-amt_coll2 + ls_final1-amt_coll3.
        ENDIF.
...
*          ls_final1-due_buk_91_120 = ls_final1-amt_coll1.
          ls_final1-due_buk_91_120 = ls_final1-amt_coll  + ls_final1-amt_coll1 + ls_final1-amt_coll2 + ls_final1-amt_coll3.
        ENDIF.


        IF ls_final1-day_coll_due GE 121 AND ls_final1-day_coll_due LE 180.
*          ls_final1-due_buk_121_180 = ls_final1-amt_coll2.
          ls_final1-due_buk_121_180 = ls_final1-amt_coll  + ls_final1-amt_coll1 + ls_final1-amt_coll2 + ls_final1-amt_coll3.
        ENDIF.


        IF ls_final1-day_coll_due GE 181.
*          ls_final1-due_buk_aft_180 = ls_final1-amt_coll3.
          ls_final1-due_buk_aft_180 = ls_final1-amt_coll  + ls_final1-amt_coll1 + ls_final1-amt_coll2 + ls_final1-amt_coll3.
        ENDIF.
