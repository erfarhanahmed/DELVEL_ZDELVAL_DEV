...
*             AND spras = 'E'.
    ...
    SELECT bukrs belnr gjahr buzei
           ...
           posn2 dmbtr wrbtr gvtyp sgtxt bupla hsn_sac lifnr
           FROM bseg
           INTO TABLE lt_bseg
           FOR ALL ENTRIES IN lt_bkpf
...
*    Bseg entries for customer
      lt_bseg_d  = lt_bseg.
      DELETE lt_bseg_d WHERE koart <> 'K'.
...
*      DELETE lt_bseg WHERE gvtyp <> 'X'  .
BREAK primus.
      DELETE lt_bseg WHERE koart NE 'S' OR EBELN = ' ' AND hsn_sac = ' '   .
...
*zno
    IF lv_mode = 'M'.
      ...
      CALL FUNCTION 'CONVERSION_EXIT_ALPHA_OUTPUT'
        EXPORTING
          input  = ls_bkpf-belnr "ls_bkpf-xblnr
        IMPORTING
...
*    gstin
    ...
    ls_final-sellerdtls-gstin =   ls_sel_addr-gstin . "'29AFQPB8708K000'." .
...
*     prddesc
      ...
      IF ls_bseg-matnr IS INITIAL.
        ...
        ls_itemlist-hsncd = ls_bseg-hsn_sac(04).
      ELSE.
...
*hsncd
        ...
        IF sy-subrc EQ 0.
          ls_itemlist-hsncd = ls_marc-steuc(06).     " '23099010'.
