...
TYPES : BEGIN OF TY_EKKK,
          EBELN TYPE EKPO-EBELN,
          EBELP TYPE EKPO-EBELP,
          MATNR TYPE EKPO-MATNR,
        END OF TY_EKKK.

DATA : IT_EKKK TYPE TABLE OF TY_EKKK,
       WA_EKKK TYPE TY_EKKK.
...
TYPES : BEGIN OF STR_DOWN,
          ...
          ERDAT            TYPE char15,  """" NC
          FTRMI            TYPE char15,  "STRING,
          FIRST_ISSUE_DATE TYPE char15, "STRING,
          LAST_ISSUE_DATE  TYPE char15,"STRING,
          TXT04            TYPE STRING,
          EBELN            TYPE STRING,
          MATNR1           TYPE STRING,
          AEDAT            TYPE char15, "STRING,
          KDPOS1           TYPE STRING,
          CPUDT_MKPF       TYPE char15,
          CPUTM_MKPF       TYPE STRING,
          BUDAT            TYPE char15,
          IEDZ             TYPE STRING,
          LV_LAG           TYPE STRING,
...
FORM GET_DATA .
  ...
  IF GT_MAIN1 IS INITIAL .
    MESSAGE |Do not data found| TYPE 'E'.
  ENDIF.

  IF GT_MAIN1 IS NOT INITIAL.
    SELECT A~VBELN
           A~KUNNR
           C~NAME1
           FROM VBAK AS A
...
*            a~matnr
      ...
      WHERE A~VBELN = GT_MAIN1-VBELN
      AND   A~WERKS = 'PL01'
      AND   A~ERDAT = GT_MAIN1-ERDAT
      AND   A~POSNR = GT_MAIN1-POSNR
      AND   C~LFSTA NE 'C'
      AND   C~GBSTA NE 'C'
      AND   C~LFGSA NE 'C'
      AND   D~ETTYP =  'CP'.
  ...
  SELECT A~EBELN
         ...
         FOR ALL ENTRIES IN GT_MAIN1
         WHERE A~VBELN = GT_MAIN1-VBELN
         AND   A~VBELP = GT_MAIN1-POSNR.
*         AND   matnr = gt_main1-matnr.
  SORT IT_EKKN BY VBELN VBELP EBELN EBELP MATNR.
  DELETE ADJACENT DUPLICATES FROM IT_EKKN COMPARING VBELN VBELP EBELN EBELP MATNR.
...
*  LOOP AT gt_main INTO wa_main.
  LOOP AT GT_MAIN1 INTO WA_MAIN1.                                 """LOOP FOR ALL PENDIND SALES ORDER
    ...
    READ TABLE IT_AFPO INTO WA_AFPO WITH KEY AUFNR = WA_DATA-AUFNR.
    IF SY-SUBRC = 0.
      WA_DATA-AFPO_MATNR = WA_AFPO-MATNR.
    ENDIF.

*    READ TABLE gt_main1 INTO wa_main1 WITH KEY vbeln = wa_main-vbeln posnr = wa_main-posnr.
*
*    wa_data-kdauf  = wa_main1-vbeln.
*    wa_data-kdpos  = wa_main1-posnr.
*    wa_data-matnr  = wa_main1-matnr.
    ...
    READ TABLE IT_CAUFV1 INTO WA_CAUFV1 WITH KEY AUFNR = WA_DATA-AUFNR .
    IF SY-SUBRC = 0.
...
* IMPLEMENT SUITABLE ERROR HANDLING HERE
    ...
    IF WA_DATA-AUFNR IS NOT INITIAL.
...
*       EARLIEST         =
    ...
    """"  """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""Below logic for Purchase Order
*    READ TABLE it_ekkn INTO wa_ekkn WITH KEY vbeln = wa_data-kdauf vbelp = wa_data-kdpos." matnr = wa_data-matnr.
*    IF sy-subrc = 0.
...
*    READ TABLE it_ekkk INTO wa_ekkk WITH KEY ebeln = wa_data-ebeln ebelp = wa_data-kdpos1.
*    IF sy-subrc =  0.
*      wa_data-matnr1  = wa_ekkk-matnr.
*    ENDIF.
*
...
*    ENDIF.
    ...
    CONCATENATE WA_DATA-IEDZ+0(2) WA_DATA-IEDZ+2(2) WA_DATA-IEDZ+4(2)  INTO WA_DATA-IEDZ.
    ...
    CLEAR :WA_DATA,WA_CAUFV1,LV_VDATUM,LV_VEZEITERF,LV_LOG_DATE,LV_LOG_TIME.
  ENDLOOP.
  SORT IT_DATA.
  ...
  SORT IT_DATA BY KDAUF KDPOS .

  LOOP AT GT_MAIN INTO WA_MAIN.                                       """LOOP FOR PRODUCTION ORDER
    ...
    READ TABLE  IT_MSEG_TEMP INTO LS_MSEG_TEMP WITH KEY AUFNR = WA_DATA-AUFNR .
    IF SY-SUBRC = 0.
      WA_DATA-FIRST_ISSUE_DATE = LS_MSEG_TEMP-BUDAT_MKPF.
    ENDIF.

    READ TABLE  IT_MSEG_TEMP1 INTO LS_MSEG_TEMP1 WITH KEY AUFNR = WA_DATA-AUFNR .
    IF SY-SUBRC = 0.
      WA_DATA-LAST_ISSUE_DATE = LS_MSEG_TEMP1-BUDAT_MKPF.
    ENDIF.

    READ TABLE IT_CAUFV_NEW INTO WA_CAUFV_NEW WITH KEY AUFNR = WA_DATA-AUFNR .
    IF  SY-SUBRC = 0.
      LV_OBJNR = WA_CAUFV_NEW-OBJNR.
      WA_DATA-ERFZEIT = WA_CAUFV_NEW-ERFZEIT.
...
* IMPLEMENT SUITABLE ERROR HANDLING HERE
    ...
    LV_VDATUM    = WA_DATA-VDATUM.
    LV_VEZEITERF = WA_DATA-VEZEITERF.
    LV_LOG_DATE  = WA_DATA-LOG_DATE.
    LV_LOG_TIME  = WA_DATA-LOG_TIME.
    """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
*    DATA : lv_date  TYPE p,
*           lv_date1 TYPE string,
*           lv_time  TYPE p,
...
*       EARLIEST         =
    ...
    CLEAR :WA_DATA.
  ENDLOOP.
  SORT IT_DATA BY KDAUF KDPOS AUFNR.
  DELETE ADJACENT DUPLICATES FROM IT_DATA COMPARING KDAUF KDPOS AUFNR.
  CLEAR: LV_VDATUM ,LV_VEZEITERF, LV_LOG_DATE ,LV_LOG_TIME.

  LOOP AT IT_EKKN INTO WA_EKKN.                                   """LOOP FOR PURCHASE ORDER
    WA_DATA-EBELN   = WA_EKKN-EBELN.
    WA_DATA-KDPOS1  = WA_EKKN-EBELP.
...
*
  ...
  SORT IT_DATA BY KDAUF KDPOS EBELN KDPOS1 AUFNR.
  DELETE ADJACENT DUPLICATES FROM IT_DATA COMPARING KDAUF KDPOS EBELN KDPOS1 AUFNR.

  LOOP AT IT_DATA INTO WA_DATA WHERE EBELN IS NOT INITIAL .
    READ TABLE IT_DATA INTO DATA(WA_DATA1) WITH KEY KDAUF = WA_DATA-KDAUF
                                                    KDPOS = WA_DATA-KDPOS
                                                    AUFNR = ' '
                                                    EBELN = ' '.

    IF SY-SUBRC IS INITIAL.
      IF WA_DATA1-AUFNR IS INITIAL AND WA_DATA1-EBELN IS INITIAL.
        DELETE IT_DATA WHERE AUFNR IS INITIAL AND EBELN IS INITIAL
                        AND KDAUF = WA_DATA-KDAUF AND KDPOS = WA_DATA-KDPOS.
      ENDIF.
    ENDIF.
...
*      WA_DOWN-AEDAT               =    WA_DATA-AEDAT             .
      ...
      IF WA_DATA-AEDAT   IS NOT INITIAL .
      ...
      IF WA_DATA-KDPOS1 IS NOT INITIAL.
...
*      WA_DOWN-BUDAT               =    WA_DATA-BUDAT             .
      ...
      IF WA_DATA-VDATUM IS NOT INITIAL.
        CALL FUNCTION 'CONVERSION_EXIT_IDATE_OUTPUT'
          EXPORTING
      ...
      IF WA_DOWN-LOG_DATE IS NOT INITIAL.
        CONCATENATE WA_DOWN-LOG_DATE+0(2) WA_DOWN-LOG_DATE+2(3) WA_DOWN-LOG_DATE+5(4)
                        INTO WA_DOWN-LOG_DATE SEPARATED BY '-'.
      ENDIF.

      SHIFT WA_DOWN-LOG_DATE LEFT DELETING LEADING space.

      IF WA_DATA-LOG_TIME IS NOT INITIAL.
      ...
      IF WA_DATA-ERDAT IS NOT INITIAL .
        ...
        ENDIF.
      ENDIF.
       SHIFT WA_DOWN-ERDAT LEFT DELETING LEADING space.


      IF WA_DATA-FTRMI IS NOT INITIAL .
        ...
        ENDIF.
      ENDIF.
       SHIFT WA_DOWN-FTRMI LEFT DELETING LEADING space.
      ...
      ENDIF.
       SHIFT WA_DOWN-FIRST_ISSUE_DATE LEFT DELETING LEADING space.
      ...
      ENDIF.
      SHIFT WA_DOWN-LAST_ISSUE_DATE LEFT DELETING LEADING space.
      ...
      ENDIF.
       SHIFT WA_DOWN-REF_TIME LEFT DELETING LEADING space.
...
FORM FILL_FIELDCAT .
  ...
  WA_FIELDCAT-SELTEXT_L = 'SO Material' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Prod Order' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Prod Order Material' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Prod Order Created Date' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Release Date' .
  WA_FIELDCAT-COL_POS = '8'.
  APPEND WA_FIELDCAT TO IT_FIELDCAT .
  CLEAR WA_FIELDCAT.

  WA_FIELDCAT-FIELDNAME = 'FIRST_ISSUE_DATE' .
  WA_FIELDCAT-SELTEXT_L = '1st Issue Date' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Last Issue Date' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'System Status' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'PO Material' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'PO Creation Date' .
...
*  CLEAR wa_fieldcat.
  ...
  WA_FIELDCAT-SELTEXT_L = 'GRN Time' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Assembly Completion Date' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Assembly Time Entered' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Assembly Lag' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Final QA Completation Date' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Final QA Time Completation Time' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Final QA Lag' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Logistic Receipt  Date' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Logistic Time Entered' .
  ...
  WA_FIELDCAT-SELTEXT_L = 'Logistic lag' .
...
* Implement suitable error handling here
  ...
  LV_FILE = 'ZSD_LAG_PENDING_SO.TXT'.
