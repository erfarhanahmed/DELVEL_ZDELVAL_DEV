...
*&
*&---------------------------------------------------------------------*
REPORT zsufi_sales_register_new1.
TYPE-POOLS:slis.
TABLES : bkpf, bseg.
DATA:
...
TYPES:
  BEGIN OF t_final,
    ...
    bldat      TYPE string,"char10, "bkpf-budat,
...
SELECT-OPTIONS: so_belnr FOR tmp_belnr,
                so_budat FOR tmp_budat ,"DEFAULT '20230401' TO sy-datum,
                so_gjahr FOR tmp_gjahr DEFAULT '2023' OBLIGATORY,
                s_blart for bkpf-blart,       " Abhishek (27.03.2018)
                SO_KUNNR FOR BSEG-KUNNR.
...
*  ENDIF.
  ...
  IF NOT lt_accounting_doc_hdr IS INITIAL.
    ...
    SELECT bukrs
      ...
      and belnr = lt_accounting_doc_hdr-belnr
...
*BREAK primus.
  LOOP AT lt_accounting_doc_item INTO ls_accounting_doc_item WHERE kunnr NE space.
    ...
    ls_final-belnr1   = ls_accounting_doc_item-belnr.
...
**                                                                               gjahr = ls_accounting_doc_item-gjahr
**                                                                               xbilk = space.
***************commented by primus jjyoti on 08.01.2024*********************************************
*        READ TABLE lt_accounting_doc_item INTO ls_accounting_doc_itm1 WITH KEY belnr = ls_accounting_doc_item-belnr
*                                                                               gjahr = ls_accounting_doc_item-gjahr
*                                                                               ktosl = space
*                                                                               saknr = space.
...
*        ENDIF.
****************************************************************************************************************
        LOOP AT LT_ACCOUNTING_DOC_ITM1 INTO LS_ACCOUNTING_DOC_ITM1 WHERE BELNR = LS_ACCOUNTING_DOC_ITEM-BELNR
                                                           AND GJAHR = LS_ACCOUNTING_DOC_ITEM-GJAHR
                                                           AND KOART = 'D'.
  IF LS_ACCOUNTING_DOC_ITM1-KOART = 'D'.
    ls_final-hwbas =  ls_final-hwbas + LS_ACCOUNTING_DOC_ITM1-DMBTR .
     ls_final-fwbas =   ls_final-fwbas + LS_ACCOUNTING_DOC_ITM1-WRBTR .
...
ENDLOOP.
    ...
    IF sy-subrc IS INITIAL.
      ...
      CONCATENATE ls_final-name1 ls_final-name2 INTO ls_final-name1 SEPARATED BY SPACE.
...
*  ls_fieldcat-outputlen = '5'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'INV/CN/DN No.'(300).
  ls_fieldcat-col_pos   = gv_pos.
  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '5'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Posting Date'(307).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '5'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Reference'(104).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*
*  gv_pos = gv_pos + 1.
*  ls_fieldcat-fieldname = 'VTEXT'.50
**  ls_fieldcat-outputlen = '5'.
*  ls_fieldcat-tabname   = 'GT_FINAL'.
...
*  ls_fieldcat-outputlen = '5'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Customer VAT Number'(108).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '5'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Country'(109).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
*  ls_fieldcat-outputlen = '5'.
  ls_fieldcat-tabname   = 'GT_FINAL'.
  ls_fieldcat-seltext_m = 'Region Description'(110).
  ls_fieldcat-col_pos   = gv_pos.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND ls_fieldcat TO ct_fieldcat.
  CLEAR ls_fieldcat.
...
FORM ucomm_on_alv USING r_ucomm LIKE sy-ucomm
  ...
  DATA:
    ls_final TYPE t_final,
    lv_burks TYPE bseg-bukrs VALUE 'SU00'.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  lv_file = 'ZSU_SALES_FI.TXT'.

  CONCATENATE p_folder '\'  lv_file
    INTO lv_fullfile.

  WRITE: / 'ZSU_SALES_FI Download started on', sy-datum, 'at', sy-uzeit.
...
*INCLUDE ZSUFI_SALES_REG_FIELDNAMF_NEW1.
