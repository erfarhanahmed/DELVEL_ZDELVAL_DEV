*&---------------------------------------------------------------------*
*& Report ZGST_MM_PURCHASE_ORDER_DRVR
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZGST_MM_PURCHASE_ORDER_DRVR.
...
*** SOC COMMENTED BY SNEHAL ON 10.06.2025  *********
...
*** EOC COMMENTED BY SNEHAL ON 10.06.2025  *********

  it_ekpo = l_doc-xekpo.
  it_ekko = l_doc-xekko.

  IF it_ekko-frgke = '2'.  " CHECK FOR PO RELEASE
    perform print_adobe_form.

*** SOC COMMENTED BY SNEHAL ON 10.06.2025  *********
*    CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
*      EXPORTING
*        formname           = 'ZGST_MM_PURCHASE_ORDER'
**    *     VARIANT                  = ' '
**       DIRECT_CALL        = ' '
*      IMPORTING
*        fm_name            = fm_name
...
*************SOC Commented by Snehal 10.06.2025 *********************************************************
...
*               OBJECT_PARB                =
      ...
      ENDIF.
 "   ENDIF.   "commented By Snehal 10.06.2025    *******
...
CALL FUNCTION gv_fm_name "'/1BCDWB/SM00000156'
  EXPORTING
*   /1BCDWB/DOCPARAMS        =
    LV_EBELN                 = v_ebeln
* IMPORTING
...
*     FIELDNAMES              =
  ...
  IF sy-subrc <> 0.
    MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
       WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
  ENDIF.
ENDFORM.                    "DOWNLOAD FILE
*&---------------------------------------------------------------------*
*& Form print_adobe_form
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*& -->  p1        text
