      ...
      LOOP AT lt_mat_doc INTO ls_mat_doc WHERE ebeln = ls_purchasing_doc_item-ebeln AND  ebelp = ls_purchasing_doc_item-ebelp.
        ...
        IF sy-subrc = 0.
          ...
          IF ls_j_1ig_subcon-bwart = '541'.
            ...
            if  ls_stb-menge is NOT INITIAL .
...
*****************************************************file download ***************************************************
    ...
    wa_final-ebeln    =  ls_final-ebeln.
    wa_final-ebelp    =  ls_final-ebelp.
    wa_final-ekgrp    =  ls_final-ekgrp.
    ...
    wa_final-ch_qty   =  abs( ls_final-ch_qty ).
    IF ls_final-ch_qty < 0.
        CONDENSE wa_final-ch_qty.
        CONCATENATE '-' wa_final-ch_qty INTO wa_final-ch_qty.
      ENDIF.

    wa_final-pen_qty  =  abs( ls_final-pen_qty ).
     IF ls_final-pen_qty < 0.
        CONDENSE wa_final-pen_qty.
        CONCATENATE '-' wa_final-pen_qty INTO wa_final-pen_qty.
      ENDIF.

    wa_final-dev_crd  =  abs( ls_final-dev_crd ).
    IF ls_final-dev_crd < 0.
        CONDENSE wa_final-dev_crd.
        CONCATENATE '-' wa_final-dev_crd INTO wa_final-dev_crd.
      ENDIF.

    wa_final-sum_rs   =  abs( ls_final-sum_rs ).
    IF ls_final-sum_rs < 0.
        CONDENSE wa_final-sum_rs.
        CONCATENATE '-' wa_final-sum_rs INTO wa_final-sum_rs.
      ENDIF.
...
*ENDIF.
  ...
  DELETE it_final WHERE pen_qty = 0.
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  DISPLAY
*&---------------------------------------------------------------------*
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  IF sy-subrc = 0.
DATA lv_string_640 TYPE string.
DATA lv_crlf_640 TYPE string.
lv_crlf_640 = cl_abap_char_utilities=>cr_lf.
lv_string_640 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_640 lv_crlf_640 wa_csv INTO lv_string_640.
  CLEAR: wa_csv.
ENDLOOP.
TRANSFER lv_string_640 TO lv_fullfile.
