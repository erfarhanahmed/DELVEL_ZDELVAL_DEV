...
*&
*----------------------------------------------------------------------*
* PROGRAM NAME           :   ZSU_PP_COMPLIANCE
* OBJECT ID              :
* FUNCTIONAL ANALYST     : Subhashish Pande
* PROGRAMMER             : Shreya Sankpal / Nilay Brahme
...
DATA: gt_list1     TYPE vrm_values.
DATA: gwa_list1    TYPE vrm_value.
DATA: gt_values1  TYPE TABLE OF dynpread,
      gwa_values1 TYPE dynpread.

DATA: gv_selected_value1(10) TYPE c.
...
DATA:it_final TYPE STANDARD TABLE OF zsu_pp_compliance,
     wa_final TYPE zsu_pp_compliance.

DATA: it_stb TYPE STANDARD TABLE OF stpox,
      wa_stb TYPE stpox.


DATA:lf_fm_name TYPE funcname,
...
SELECTION-SCREEN BEGIN OF BLOCK b2 WITH FRAME TITLE TEXT-001.
  PARAMETERS: r_ball RADIOBUTTON GROUP rad2 DEFAULT 'X', "Local
              r_bfv  RADIOBUTTON GROUP rad2 , "USER-COMMAND flg.
              r_btt  RADIOBUTTON GROUP rad2,
              r_doto RADIOBUTTON GROUP rad2,
              r_tmbv RADIOBUTTON GROUP rad2,
              r_tri  RADIOBUTTON GROUP rad2.
...
SELECTION-SCREEN: BEGIN OF BLOCK a WITH FRAME TITLE TEXT-001.
  ...
  PARAMETERS :p_zdelsr TYPE zsu_pp_compliance-zdelsrno,
              p_ztagno TYPE zsu_pp_compliance-ztagno.
...
INITIALIZATION.
  gwa_list-key = '1)'.
  gwa_list-text = 'API 6D'.
  APPEND gwa_list TO gt_list.
  gwa_list-key = '2)'.
  gwa_list-text = 'ISO 5208'.
  APPEND gwa_list TO gt_list.
  gwa_list-key = '3)'.
  gwa_list-text = 'API 598'.
  APPEND gwa_list TO gt_list.
  gwa_list-key = '4)'.
  gwa_list-text = 'BS EN 12266-1'.
  APPEND gwa_list TO gt_list.


  gwa_list1-key = '1)'.
  gwa_list1-text = '0 to 4"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '2)'.
  gwa_list1-text = '6" to 10"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '3)'.
  gwa_list1-text = '12" to 18"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '4)'.
  gwa_list1-text = '20" and ABOVE'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '5)'.
  gwa_list1-text = '0 to 2"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '6)'.
  gwa_list1-text = '2-1/2" to 6"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '7)'.
  gwa_list1-text = '8" to 12"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '8)'.
  gwa_list1-text = '14" to 24"'.
  APPEND gwa_list1 TO gt_list1.
  gwa_list1-key = '9)'.
  gwa_list1-text = '24" and ABOVE'.
  APPEND gwa_list1 TO gt_list1.
...
FORM get_data .
  ...
  IF p_aufnr IS NOT INITIAL.
    ...
    SELECT SINGLE kdauf kdpos FROM afpo INTO ( v_vbelv , v_posnv )
                                          WHERE aufnr = p_aufnr
    AND dwerk EQ 'SU01' .
...
*****POPULATING DATA INTO IT_FINAL.
  ...
  wa_final-r_ball = r_ball.
  wa_final-r_bfv = r_bfv.
  wa_final-r_btt = r_btt.
  wa_final-r_doto = r_doto.
  wa_final-r_tmbv = r_tmbv.
  wa_final-r_tri = r_tri.
  wa_final-ztest_std = list.
  wa_final-zsu_series = list1.
...
*     ev_funcname_inbound =
  ...
  DATA:lv_sfpdocparams TYPE fpformoutput,
       lv_fp_docparams TYPE sfpdocparams.

  CALL FUNCTION lf_fm_name     "'/1BCDWB/SM00000154'
    EXPORTING
  ...
  DATA: lt_pdf_data     TYPE TABLE OF solix,
        lv_length       TYPE i,
...
*     bin_filesize            =                   " File length for binary files
    ...
    CHANGING
      data_tab                = lt_pdf_data                    " Transfer table
    EXCEPTIONS
