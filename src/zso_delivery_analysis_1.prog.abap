*&---------------------------------------------------------------------*
*& Report ZSO_DELIVERY_ANALYSIS_1
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zso_delivery_analysis_1.
...
DATA: it_vbak  TYPE TABLE OF ty_vbak,
      ...
      it_aufm1  TYPE TABLE OF ty_aufm,
      wa_aufm1  TYPE          ty_aufm,
...
SELECT-OPTIONS : s_vbeln FOR vbak-vbeln.                       "vbak-vbeln,
*                 S_edatu FOR vbep-edatu."commented by mohsin
...
SELECTION-SCREEN COMMENT /1(63) TEXT-006.
...
*           AND erdat IN S_edatu" commented by mohsin
           AND vkorg = p_vkorg.
 ...
 IF it_vbak IS NOT INITIAL.
    ...
    SELECT vbeln
           ...
           WHERE vbeln = IT_VBAK-VBELN
             AND etenr = '0001'.
*             AND edatu IN S_EDATU.  "Added By Nilay B. on 21 .12.2023 "commented by mohsin
...
****added by primus jyoti on 25.04.2024*******************
*      SELECT mblnr
*           mjahr
*           zeile
*           budat
...
*           kdpos
*           menge
*           aufnr FROM aufm INTO TABLE it_aufm
*           FOR ALL ENTRIES IN it_afpo
*           WHERE aufnr = it_afpo-aufnr
*             And kdauf = it_afpo-kdauf
*             AND kdpos = it_afpo-kdpos
*             AND bwart = '101'
*             AND lgort IN ('TPI1','FG01','KFG0','KTPI')   ""ADDING SLOC BY PRANIT 02.01.2024
*             AND WERKS = 'PL01'.
******************aded by jyotio n 2801.2024*********************
  SELECT mblnr
           mjahr
           zeile
           budat
           ...
           kdpos
           menge
           aufnr FROM aufm INTO TABLE it_aufm1
           FOR ALL ENTRIES IN it_afpo
           WHERE aufnr = it_afpo-aufnr
             And kdauf = it_afpo-kdauf
             AND kdpos = it_afpo-kdpos
...
*    ENDIF.
    ...
    READ TABLE it_afpo INTO wa_afpo WITH KEY kdauf = wa_final-vbeln kdpos = wa_final-posnr aufnr = wa_aufm-aufnr.
    IF sy-subrc = 0.
...
*                      '11'   'MRP_DT'          'IT_FINAL'      'MRP Date'                       '15' ,
                      '11'   'MBLNR'           'IT_FINAL'      'Material Document'              '15' ,
                      '12'   'LGORT'           'IT_FINAL'      'Storage Location'              '15' ,
                      '13'   'MENGE'           'IT_FINAL'      'Quantity'              '15' .
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  GET_DISPLAY
*&---------------------------------------------------------------------*
...
* Implement suitable error handling here
  ...
  lv_file = 'ZDEL_PERFORM_1.TXT'.

  CONCATENATE p_folder '/' lv_file
  INTO lv_fullfile.
* BREAK primus.
  WRITE: / 'ZDEL_PERFORM_1 Download started on', sy-datum, 'at', sy-uzeit.
  OPEN DATASET lv_fullfile
  FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF sy-subrc = 0.
DATA lv_string_660 TYPE string.
DATA lv_crlf_660 TYPE string.
lv_crlf_660 = cl_abap_char_utilities=>cr_lf.
lv_string_660 = hd_csv.
LOOP AT it_csv INTO wa_csv.
CONCATENATE lv_string_660 lv_crlf_660 wa_csv INTO lv_string_660.
  CLEAR: wa_csv.
ENDLOOP.
*TRANSFER lv_string_930 TO lv_fullfile.
TRANSFER lv_string_660 TO lv_fullfile.
