...
SELECTION-SCREEN: BEGIN OF BLOCK B1 WITH FRAME TITLE TEXT-001.
  ...
  PARAMETERS    : P_DWERK TYPE AFPO-DWERK OBLIGATORY DEFAULT 'PL01'.
...
FORM GET_DATA .
  ...
  SELECT AUFNR
         ...
         KUNNR2
         DNREL
         LGORT FROM AFPO INTO TABLE IT_AFPO
...
FORM SORT_DATA .
  LOOP AT IT_AFPO INTO WA_AFPO.
    ...
    WA_FINAL-V_POSNR  = WA_AFPO-KDPOS.


    WA_FINAL-PEN_QTY = WA_FINAL-PSMNG - WA_FINAL-WEMNG.


    SELECT SINGLE MENGE FROM MSEG      "ECC
   INTO @DATA(GV_MENGE)
   WHERE AUFNR = @WA_FINAL-AUFNR
   AND BWART = '101'.
...
*      SELECT SINGLE POSNR FROM VBAP INTO V_POSNR WHERE MATNR = WA_AFPO-MATNR "added BY PJ     "commented in ecc
...
*  Title
  WA_HEADER-TYP  = 'H'.
  WA_HEADER-INFO = 'Production Order Detail Report'.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  LV_FILE = 'ZORDER_DETAIL.TXT'.


  CONCATENATE P_FOLDER '/' LV_FILE
    INTO LV_FULLFILE.

  WRITE: / 'ZPRODUCTION ORDER DETAIL REPORT started on', SY-DATUM, 'at', SY-UZEIT.
