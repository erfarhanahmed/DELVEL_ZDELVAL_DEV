...
*    SELECT a~vbeln,
*           A~netwr
*      FROM vbak as a INNER JOIN vbrk as b
*      on a~vbeln eq b~vbeln
...
*      INTO TABLE @DATA(it_vbak).
    ...
    SORT IT_DATA1 BY VBELN.
    DELETE ADJACENT DUPLICATES FROM IT_DATA1 COMPARING VBELN.
...
*     MESSAGE |NO DATA FOUND| TYPE 'E'.
  ...
  IF IT_DATA1 IS NOT INITIAL.
    ...
    DATA(IT_VBAP_NEW) = IT_VBAP.
    DELETE IT_VBAP_NEW WHERE DELDATE = '00000000'.

    SORT IT_VBAP_NEW ASCENDING BY DELDATE.
  ENDIF.
...
INTO TABLE @DATA(IT_VBPA_1).
    ...
    IF IT_VBPA IS NOT INITIAL.
      DATA(IT_VBPA_NEW_1) = IT_VBPA_1[].

      DELETE IT_VBPA_NEW_1 WHERE PARVW NE 'WE'.
      ...
      SELECT SPRAS,
             ...
             BEZEI
          FROM T005U
          FOR ALL ENTRIES IN @IT_VBPA_NEW_1
          WHERE BLAND = @IT_VBPA_NEW_1-REGION
          AND SPRAS EQ @C_SPRAS
          AND LAND1 EQ @IT_VBPA_NEW_1-COUNTRY
      ...
      FROM T005T
      FOR ALL ENTRIES IN @IT_VBPA_NEW_1
      WHERE LAND1 = @IT_VBPA_NEW_1-COUNTRY
      AND SPRAS EQ @C_SPRAS
*      AND land1 EQ @c_land
  ...
  LOOP AT IT_DATA1 INTO DATA(WA_DATA).                                      """"Loop for pending sales order wise data
    WA_FINAL-VBELN   =   WA_DATA-VBELN.

    READ TABLE IT_VBAP_NEW INTO DATA(WA_VBAP_NEW) WITH KEY VBELN = WA_FINAL-VBELN.
    IF SY-SUBRC = 0.
      WA_FINAL-DELDATE =   WA_VBAP_NEW-DELDATE.
    ENDIF.

    READ TABLE IT_DATA INTO DATA(LS_DATA) WITH KEY VBELN = WA_FINAL-VBELN.
    IF SY-SUBRC = 0.
      WA_FINAL-ERDAT     =   LS_DATA-ERDAT.
      WA_FINAL-ERNAM     =   LS_DATA-ERNAM.
      WA_FINAL-AUART     =   LS_DATA-AUART.
      WA_FINAL-BSTDK     =   LS_DATA-BSTDK.
      WA_FINAL-BSTNK     =   LS_DATA-BSTNK.
      WA_FINAL-VKBUR     =   LS_DATA-VKBUR.
      WA_FINAL-INCO1     =   LS_DATA-INCO1.
      WA_FINAL-INCO2     =   LS_DATA-INCO2.
      WA_FINAL-ZTERM     =   LS_DATA-ZTERM.
      WA_FINAL-VDATU     =   LS_DATA-VDATU.
      WA_FINAL-ORDER_AMT =   LS_DATA-NETWR.
    ENDIF.

    READ TABLE IT_VBPA_NEW INTO DATA(WA_VBPA_NEW) WITH KEY VBELN = WA_FINAL-VBELN PARVW = 'AG'.
    IF SY-SUBRC = 0.
      WA_FINAL-KUNNR = WA_VBPA_NEW-KUNNR.
      WA_FINAL-NAME1 = WA_VBPA_NEW-NAME1.
    ENDIF.
...
*      wa_final-name1 = wa_vbpa-name1.
    ...
    READ TABLE IT_VBPA_NEW_1 INTO DATA(WA_T005U) WITH KEY VBELN = WA_FINAL-VBELN.
    IF SY-SUBRC = 0.
      WA_FINAL-BEZEI = WA_T005U-BEZEI.
    ENDIF.


    READ TABLE IT_T005T INTO DATA(WA_T005T) WITH KEY LAND1 = WA_VBPA-COUNTRY.
    IF SY-SUBRC = 0.
...
******************************************************************************
    ...
    WA_FINAL-PENDSO_AMT = WA_FINAL-ORDER_AMT - LV_NETWR.                                """ Pendso amt = order amt - netwr

    READ TABLE IT_VBRP_NEW INTO DATA(WA_VBRP_NEW) WITH KEY AUBEL = WA_FINAL-VBELN.
    IF SY-SUBRC = 0.
      IF WA_VBRP_NEW-FKSTO EQ 'X'.
        WA_FINAL-PENDSO_AMT = WA_FINAL-ORDER_AMT.
      ENDIF.
    ENDIF.
    ...
    IF WA_FINAL-ORDER_AMT < 0.
      CALL FUNCTION 'CLOI_PUT_SIGN_IN_FRONT'                      """fm use negative sign in front
        CHANGING
          VALUE = WA_FINAL-ORDER_AMT.
    ENDIF.
    ...
    IF ( WA_FINAL-AUART EQ 'US04' OR WA_FINAL-AUART EQ 'US05' ) AND WA_VBRP_NEW-FKSTO EQ 'X' .       """""

    ELSE.
      APPEND WA_FINAL TO IT_FINAL.
      CLEAR :WA_FINAL,LV_NETWR,LV_DATA.
    ENDIF.
  ENDLOOP.
  ...
  IF P_DOWN = 'X'.
    LOOP AT IT_FINAL INTO WA_FINAL.
      ...
      WA_DOWN-ORDER_AMT         =  WA_FINAL-ORDER_AMT       .
...
FORM FCT .
  ...
  PERFORM GT_FIELDCATLOG USING :
         ...
         'ORDER_AMT'    'IT_FINAL'     'Order Amount'              '26',
...
* Implement suitable error handling here
  ...
  LV_FILE = 'ZUS_VA05N_PENDING_SO.TXT'.
